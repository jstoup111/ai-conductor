...
```mermaid
graph TD
  subgraph conductor["conduct-ts engine"]
    CR["Conductor.run()<br/>auto-mode build-failure block<br/>(conductor.ts ~926-945)"]
    DR["daemon-runner.ts<br/>feature-outcome handling"]
    DLOOP["daemon.ts<br/>startup reconcile + poll loop"]
    HALT["HALT path<br/>.pipeline/HALT + halt-reconciliation"]
    FIN["/finish step<br/>creates/reuses PR → pr_url"]
  end
  ...
  subgraph new["new modules (this feature)"]
    ...
    PROPS["pr-labels.ts<br/>shared gh PR primitives:<br/>ensureLabel / add / remove /<br/>prMergeState / findOrCreatePr /<br/>comment / setReady"]
    WATCH[("mergeable-watch.jsonl<br/>per-repo tracking registry")]
  end

  GH["GitHub<br/>(via gh CLI)"]

  CR -->|"build failed irrecoverably (auto)"| ESC
  CR -.->|"writes HALT first (unchanged)"| HALT
  ESC -->|"push, draft PR, comment, label"| PROPS

  DR -->|"outcome = done + pr_url"| CLEAR
  ...
  SWEEP -->|"prMergeState; add/remove mergeable; prune"| PROPS

  FIN -->|"PR url reused by ESC/CLEAR on same branch"| PROPS
  HALT -.->|"re-kick on base advance → build may now succeed"| DR
...
### Legend
- **Green** = new modules added by this feature. **Orange cylinder** = new persisted state.
...
sequenceDiagram
  ...
  participant PR as pr-labels (gh)
  participant GH as GitHub

  CR->>CR: build step retries exhausted
  CR->>HALT: write HALT + state (FIRST, unchanged)
  CR->>ESC: escalateBuildFailure(...) [step===build, mode===auto]
  ESC->>ESC: derive branch+base; count commits
  alt zero commits (FR-6)
    ESC-->>CR: return {} (no GitHub surface)
  else commits exist (FR-2)
    ESC->>PR: git push -u origin <branch>
    alt push fails (FR-7)
      PR-->>ESC: error (logged, swallowed)
      ESC-->>CR: return {} (run still parks)
    else pushed
      ESC->>PR: findOrCreatePr(--draft) (FR-4/FR-5)
      PR->>GH: gh pr view||create --draft
      ESC->>PR: ensureLabel+addLabel needs-remediation (FR-2)
      ESC->>PR: comment(reason+error) (FR-3, priority artifact)
...
sequenceDiagram
  ...
  DR->>DR: feature outcome = done, has pr_url? (FR-9)
  DR->>CLEAR: if PR carries needs-remediation (FR-16)
  CLEAR->>PR: removeLabel needs-remediation + gh pr ready (best-effort)
  ...
  loop startup + after each feature + per poll tick (FR-14)
    ...
    SWEEP->>PR: prMergeState(prUrl) → state,mergeable,checks,labels
    PR->>GH: gh pr view --json state,mergeable,statusCheckRollup,labels
    alt MERGED/CLOSED/404 (FR-13)
      ...
      SWEEP->>PR: ensureLabel+addLabel mergeable
    else not mergeable / UNKNOWN (FR-11)
