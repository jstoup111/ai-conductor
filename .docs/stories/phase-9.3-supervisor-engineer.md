# Stories: Phase 9.3 — Supervisor / Engineer (capstone)

**Status:** Accepted
**Source PRD:** `.docs/specs/2026-06-25-phase-9.3-supervisor-engineer.md`
**Complexity tier:** L
**Persona note:** "I" is the **human operator** (James) driving `conduct engineer` — bringing ideas,
confirming routing, reviewing/merging spec PRs. The **engineer** is the non-autonomous planning
service. Downstream, the **per-project daemon** consumes merged specs (unchanged here). The engineer
...
## Story: `conduct engineer` loop starts and loads registry + store

**Requirement:** FR-1
As the operator, I want `conduct engineer` to start a session that loads the registry and opens the
engineer store, so routing and the flywheel have their inputs from the first idea.

### Acceptance Criteria
#### Happy Path
- Given a populated registry (9.2) and engineer store (9.1), when I run `conduct engineer`, then it loads
  the registry via the `RegistryReader` and opens the store via the `EngineerStoreReader`, reports the
  count of known projects, and enters the interactive loop ready for the first idea.

#### Negative Paths
- Given the registry file is **absent**, when I start `conduct engineer`, then it starts with **zero
  known projects** (every idea will route to create-new, FR-4) rather than crashing — and says so.
- Given the engineer store is **absent or empty**, when I start, then the flywheel read (FR-5) is a
...
### Done When
- [ ] `conduct engineer` loads registry + store, prints project count, enters the loop.
...
**Requirement:** FR-3
As the operator, I want the engineer to propose a target project for my idea and require my
confirmation before it authors anything, so nothing is written to a repo I didn't approve.

### Acceptance Criteria
#### Happy Path
- Given an idea and a registry with candidate projects, when routing runs, then the engineer proposes
...
- Given a proposed target, when I **decline without choosing** (or send empty confirmation), then
  **no authoring happens** — the engineer re-prompts for a target or the next idea. (No "silent yes":
  confirmation must be affirmative.)
- Given I redirect to a name **not in the registry**, when validated, then the engineer rejects it and
...
- Given the idea is ambiguous and **multiple projects score similarly**, when proposing, then the
  engineer surfaces the top candidates for me to choose rather than silently picking one.
...
**Requirement:** FR-4
As the operator, I want the engineer to offer to create a new project when no registered one fits, so a
genuinely new idea isn't forced into the wrong repo.

### Acceptance Criteria
#### Happy Path
- Given an idea with **no fitting registered project**, when routing runs, then the engineer offers to
...
- Given `create` **fails** (e.g. target dir non-empty — 9.2 FR-7), when it errors, then the engineer
...
- Given create succeeds but registry write is then unreadable, when re-resolving the target, then
  the engineer stops with a clear error rather than authoring into an unregistered path.
...
**Requirement:** FR-5
As the operator, I want the engineer to read the 9.1 store and inject the **relevant** prior lessons
...
- Given a confirmed target project and a store with signals, when planning context is assembled,
  then the engineer selects lessons **relevant** to that project and similar features (per the chosen
...
- Given the store has signals but **none relevant** to this project/feature, when selecting, then
  the engineer injects **nothing** (or an explicit "no prior lessons") rather than padding planning
...
**Requirement:** FR-6
As the operator, I want the engineer to author the spec by running the existing DECIDE skills
...
### Acceptance Criteria
#### Happy Path
- Given a confirmed target repo, when authoring runs, then the engineer operates **with that repo as
...
- Given the target repo has **uncommitted changes** on its current branch, when authoring starts,
  then the engineer does **not** silently stash/clobber them — it branches from a clean ref or errors
  clearly, leaving the operator's working tree intact.
- Given the `spec/<feature>` branch **already exists**, when creating, then the engineer does not
...
- Given authoring, when it runs, then **no build/test-implementation step executes** — the engineer's
...
**Requirement:** FR-7
As the operator, I want the engineer to open a PR with the authored spec and stop there, so my merge is
the approval and nothing ships without me.

### Acceptance Criteria
#### Happy Path
- Given a `spec/<feature>` branch with committed artifacts, when handoff runs, then the engineer opens
  a **PR** in the target repo (reusing existing PR machinery), reports the PR URL, and returns to
  the loop.

#### Negative Paths
- Given the PR is opened, when handoff completes, then the engineer **does not merge it** — assert no
  `gh pr merge` / merge API call is ever issued by the engineer (the merge is the human's act).
- Given the PR is opened, when handoff completes, then the engineer **does not run any build/pipeline**
  — there is no transition from "PR opened" to "implementation" inside the engineer.
- Given the target repo has **no remote / no GitHub** (e.g. a freshly `create`d local-only repo),
  when handoff runs, then the engineer reports that the spec is committed on the branch and a PR could
...
- [ ] **No merge call** and **no build** issued by the engineer under any path (asserted).
...
## Story: Engineer may launch a daemon detached, but never manages one

**Requirement:** FR-8 (revised per ADR-005 operator refinement)
As the operator, I want the engineer to optionally *launch* a project's daemon as a convenience, but
never manage it, so daemons stay independent and fault-isolated.

### Acceptance Criteria
#### Happy Path
- Given a target project, when I ask the engineer to start its daemon, then the engineer launches it as a
  **detached, fire-and-forget** process and returns to the loop — holding **no** retained handle,
  IPC channel, or supervision over it.
- Given the engineer did not ask to launch a daemon, when it finishes an idea, then it takes **no**
  daemon action at all — authoring a spec never implies launching a daemon.

#### Negative Paths
- Given the engineer launches a daemon, when it runs end to end, then it **never stops, restarts,
...
- Given a daemon the engineer launched, when the just-authored spec PR is still **unmerged**, then the
...
- Given the spec PR is merged later out-of-band, when that happens, then the engineer has **no callback
  or watcher** acting on it (the engineer takes no further action on that feature; the daemon consumes
  it independently).

### Done When
- [ ] Engineer MAY launch a daemon **detached** (no retained handle/IPC, no supervision/control-state
      write); never stops/restarts/configures/supervises one.
- [ ] A engineer-launched daemon builds merged specs only — no engineer→build path (asserted).
- [ ] No post-merge watcher/callback in the engineer.
...
**Requirement:** FR-9
As the operator, I want the engineer to report aggregate token spend and kickback/halt/retry rates from
the store, so I have a governor view — without it gating or brokering execution.

### Acceptance Criteria
#### Happy Path
- Given a store with signals, when I request the governor report, then the engineer computes and prints
...
**Requirement:** FR-10
As the operator, I want it to be structurally impossible for the engineer to cause a build without my
merged spec PR, so non-autonomy holds by construction.

### Acceptance Criteria
#### Happy Path
- Given the entire engineer codebase, when an idea is processed, then **every** path from idea to any
  build passes through a human-merged spec PR — there is no code path `engineer → build`.

#### Negative Paths
- Given the engineer, when it runs, then it issues **no auto-merge** of its own spec PRs (asserted at
  every handoff path, not just the common one).
- Given the engineer might **propose a harness self-edit**, when it does, then the edit is emitted as a
  **PR through the existing validation / no-auto-merge gates** — never auto-applied to the working
  tree or auto-merged.
- Given an attempt (test or future code) to call a build/merge entry point from within the engineer,
  when exercised, then it is absent/guarded — a test asserts the engineer module does not import or
  invoke the build/pipeline or merge entry points.

### Done When
- [ ] No `engineer → build` path; no auto-merge on any handoff path.
- [ ] Self-edits are propose-only PRs through existing gates, never auto-applied.
- [ ] Test: structural assertion that engineer neither invokes build/pipeline nor auto-merges; self-edit
...
on project A can never create branches/PRs or edits in project B (or the engineer's own repo).
...
- Given the engineer process's own cwd is repo C (the harness repo), when authoring for A runs, then no
  branch/commit/PR is created in C — authoring never leaks into the engineer's own repo.
- Given a registry `path` that **no longer exists** on disk, when resolving the target, then the
  engineer errors clearly **before** writing anything (it does not fall back to cwd and author into the
  wrong repo — the Phase-9 recurring failure mode).

### Done When
- [ ] All writes confined to the resolved target repo path.
- [ ] Sibling repo B and the engineer's own repo C are provably untouched after authoring A.
...
- [ ] Test: two real temp repos → author A, assert B unchanged; missing-path target → pre-write
      error; assert no writes in the engineer's own repo.

---

## Story: Flywheel is measurable across successive engineer-planned features

**Requirement:** FR-12
As the operator, I want the engineer to compute whether kickback/halt/retry rates fall across
successive engineer-planned features, so I can tell the loop is learning, not just accumulating noise.

### Acceptance Criteria
#### Happy Path
- Given store signals tagged to a sequence of engineer-planned features, when the trend is computed,
  then the engineer reports the kickback/halt/retry rate **per feature in order** and a trend
  (improving / flat / worsening) per 9.1's metric definition.

#### Negative Paths
- Given **fewer than two** engineer-planned features in the store, when computing the trend, then it
...
- Given signals from **non-engineer-planned** work in the same store, when computing the engineer-flywheel
  trend, then they are **excluded** (the metric measures engineer-planned features specifically), so the
  trend isn't contaminated. **Note (cross-phase gap, conflict-check):** 9.1's signal schema has **no
  provenance field** distinguishing engineer-planned from daemon-only work. The engineer therefore
  identifies engineer-planned features from its **own record of authored `(project, feature)` keys**
...
- Given a engineer-planned feature that has **not yet produced a signal** (spec merged but daemon not
...
- [ ] Engineer-planned set derived from the engineer's authored `(project, feature)` keys, intersected with
      store signals — no dependency on a 9.1 schema provenance field.
- [ ] <2 features → "insufficient data"; zero-denominator → no div-by-zero; non-engineer work excluded.
