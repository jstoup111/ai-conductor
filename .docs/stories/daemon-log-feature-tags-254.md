...
## Story 1: Attribute every active-feature daemon line

As an operator monitoring or reviewing daemon output, I want every line emitted while a feature is active to identify that feature so that interleaved and historical activity is attributable at a glance.

### Acceptance Criteria

#### Happy Path

- Given a daemon is executing a feature with slug `daemon-logs-tag-current`, when any line is emitted from that feature's lifecycle—including setup output, step events, warnings, retries, and subprocess diagnostics—then the live and persisted forms begin with `[daemon][daemon-logs-tag-current]` before the line content.
...
- [ ] A focused test exercises representative setup, structured-event, warning, retry, and subprocess-diagnostic lines through the feature-owned logging boundary and observes `[daemon][<feature>]` in both live and persisted output.
