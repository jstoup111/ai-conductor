...
- The terminal renderer and the daemon log renderer both render a `github_write_credential_fallback` event as a line containing its operation, target, and reason, with no token value, no token file path, and no raw `gh` or `git` output.

**Files:** src/conductor/src/types/events.ts; src/conductor/src/engine/event-sinks.ts; src/conductor/test/engine/event-sinks.test.ts; src/conductor/src/ui/terminal-renderer.ts; src/conductor/src/daemon-cli.ts; src/conductor/test/ui/terminal-renderer.test.ts
...
2. Verify RED.
3. Implement. Extend the destination returned by `resolveRemoteGitTargets` (`src/engine/remote-git-targets.ts`) with the resolved endpoint scheme (`https` or `ssh`, derived in `repositoryFromRemoteUrl` from the `get-url --push` output). In `executeRemoteGit`, after every ref is authorized, invoke `runRemoteGit` with `credential: 'write'` and the destination's `endpoint`. Do not read bot config here. The production runner (Task 7) is the only reader, so existing tests with fake runners never depend on the operator's user config. On `GithubBotAuthRefusalError`, of any reason including `unsupported-remote-transport`, emit `github_write_credential_fallback` with the error's reason first. If the emit throws, make no operator attempt and rethrow the original `GithubBotAuthRefusalError` to the caller (never collapse it into a generic `failed` result), matching Task 5's runner contract. Otherwise invoke `runRemoteGit` exactly once more with identical args and `credential: 'operator'`. Refusal paths stay unchanged: no spawn and no fallback event.
4. Verify GREEN and commit.
...
- On a `GithubBotAuthRefusalError`, `executeRemoteGit` emits one fallback event with the error's reason first and then invokes `runRemoteGit` exactly once more with `credential: 'operator'` and identical args; if the emit throws, no operator attempt is made and `executeRemoteGit` rejects with the original typed `GithubBotAuthRefusalError` rather than returning a generic `failed` result.
...
### Task rem-prd-audit-rem-s41-1: src/conductor/src/engine/daemon-deps.ts:138-144 (makeFeatureRunnerDeps) — pass events: cfg.events into createDaemonHaltPrOperations, matching daemon-cli.ts:1711-1718; start from a failing test in src/conductor/test/engine/github-bot-fallback-wiring.test.ts that builds makeFeatureRunnerDeps with a recording emitter, drives a haltPrOperations mutation whose mocked gh transport throws GithubBotAuthRefusalError('token-unavailable') on credential 'write', and asserts one github_write_credential_fallback event (reason token-unavailable) then exactly one operator-credential attempt
**Gate:** prd-audit
**Rationale:** src/conductor/src/engine/daemon-deps.ts:138-144 builds createDaemonHaltPrOperations without events although cfg.events exists (daemon-deps.ts:94) and the sibling daemon-cli.ts:1711-1718 passes it, so the halt-presentation cleanup write (daemon-runner.ts:369) rethrows a token-unavailable GithubBotAuthRefusalError at tracker-client.ts:314-315 with no warning and no operator run; conforming wiring drift under approved ADR D9. Examined Tasks 5 and 8: their Done-when and Files cover only the runner/executeRemoteGit behavior (tracker-client.ts, remote-git-operations.ts), not daemon-deps.ts composition, so no existing task admits the edit and the fix is an appended build task. Sibling sweep: every other createDaemonHaltPrOperations/escalateBuildFailure construction (daemon-cli.ts:1711, daemon-deps.ts:261) already passes events; Task 5's runner behavior and its github-bot-write-fallback.test.ts coverage are unchanged.
**Criterion:** S4.1
**Parent task:** 5
**Done when:**
- S4.1 is satisfied by this task.
- Re-run prd-audit and confirm task rem-prd-audit-rem-s41-1 is complete.

### Task rem-prd-audit-rem-s42-1: src/conductor/test/engine/github-bot-fallback-wiring.test.ts — add the auth-refused case for the makeFeatureRunnerDeps halt-presentation runner: mocked gh transport throws GithubBotAuthRefusalError('auth-refused') on credential 'write'; assert the fallback event is emitted before exactly one operator attempt, and that a non-auth failure produces no event and no operator attempt
**Gate:** prd-audit
**Rationale:** Same root cause as S4.1 (src/conductor/src/engine/daemon-deps.ts:138 emitter-less halt-presentation runner): an auth-refused cleanup write rethrows at tracker-client.ts:315 instead of warning then retrying once as operator; the rem-s41-1 wiring fixes both rows, and this task adds the auth-refused regression case so S4.2 is pinned independently. Examined Tasks 5 and 8: neither admits daemon-deps.ts composition, hence an appended task; no existing assertion is removed or relaxed.
**Criterion:** S4.2
**Parent task:** 5
**Done when:**
- S4.2 is satisfied by this task.
- Re-run prd-audit and confirm task rem-prd-audit-rem-s42-1 is complete.

### Task rem-as-built-rem-ab1-1: src/conductor/src/engine/conductor.ts:3665-3668 (surfaceRemediationPr) — pass events: this.events into this.escalateBuildFailure (widen EscalateBuildFailureOpts at conductor.ts:1906 only if it lacks the optional events field build-failure-escalation.ts already accepts); start from a failing test in src/conductor/test/engine/github-bot-fallback-wiring.test.ts that constructs a daemon Conductor with a recording emitter and a recording escalateBuildFailure stub, triggers surfaceRemediationPr, and asserts the stub received that same emitter
**Gate:** as-built
**Rationale:** src/conductor/src/engine/conductor.ts:3665-3668 (surfaceRemediationPr) calls this.escalateBuildFailure({ projectRoot, failureReason }) without events, although the daemon root gives the Conductor featureEvents (daemon-cli.ts:1339-1344) and build-failure-escalation.ts:161-169,181-192 consumes only opts.events, so a typed bot refusal on the escalation push or guarded PR mutations is rethrown before fallback (remote-git-operations.ts:211-224, tracker-client.ts:313-321); conforming drift under approved ADR D9, no architecture decision needed. Examined Tasks 5, 8, and 10: none admits conductor.ts composition, hence an appended task. Sibling sweep: the daemon-deps.ts:261 escalateBuildFailure wrapper already passes cfg.events and is excluded.
**Governing clause:** adr-2026-09-11-github-operation-ownership D9
**Done when:**
- adr-2026-09-11-github-operation-ownership D9 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab1-1 is complete.

### Task rem-as-built-rem-ab2-5: src/conductor/src/engine/halt-marker.ts:87-113 (writeHaltMarker) — pass { ...recordRemote, events: recordRemote?.events ?? events } to writeHaltRecord so every HALT record publication uses the caller's existing emitter; start from a failing test in src/conductor/test/engine/github-bot-fallback-wiring.test.ts that calls writeHaltMarker with a recording emitter and a recordRemote whose remoteGit fake throws GithubBotAuthRefusalError on credential 'write', asserting one github_write_credential_fallback event then exactly one operator push
**Gate:** as-built
**Rationale:** src/conductor/src/engine/halt-marker.ts:87-113 forwards only the separate recordRemote argument to writeHaltRecord, and conductor.ts:2303-2315 passes this.events as marker telemetry but no recordRemote, so publishHaltRecord (halt-record.ts:177-199) calls executeRemoteGit with events undefined; the resolution watcher is built at daemon-cli.ts:1906-1910 without recordRemote, so daemon-deps.ts:592 supersedeHaltRecord has no emitter either — conforming drift under approved ADR D9. Close the class from one source: default the record-publication emitter to the marker emitter inside writeHaltMarker, which also covers the sibling callers that already pass events (task-progress.ts:584, rebase.ts:647/664/747, daemon-runner.ts:813, self-host/gate-halt.ts:42, self-host/build-auth-preflight.ts:59, provider-lifecycle.ts:206, auto-park-halt.ts:76, conductor.ts:6132); task-cli.ts:349 (skill-directed plan-gap HALT, S2.3 in-session CLI parity) passes no emitter and gets the existing startOperatorEventSpine. Found-and-excluded: rewind.ts:55 restoreHalt re-writes an already-published HALT during an operator rewind and is not cited by the review. Examined Tasks 5, 8, 10: none admits halt-marker/daemon-cli/task-cli composition, hence appended tasks; no existing assertion is removed — halt-marker tests that assert record-remote options are updated to include events, preserving Task 13's unconfigured-behavior coverage.
**Governing clause:** adr-2026-09-11-github-operation-ownership D9
**Done when:**
- adr-2026-09-11-github-operation-ownership D9 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab2-5 is complete.

### Task rem-as-built-rem-ab2-6: src/conductor/src/daemon-cli.ts:1906-1910 — construct the watcher as makeWatchHaltClearedSeam(worktreeBase, { recordRemote: { events } }) using the daemon's existing canonical emitter so the halt-resolution push at src/conductor/src/engine/daemon-deps.ts:592 reaches warning-first fallback; add a failing test in src/conductor/test/engine/github-bot-fallback-wiring.test.ts asserting the resolution record publication receives the emitter and emits the fallback before one operator push
**Gate:** as-built
**Rationale:** src/conductor/src/engine/halt-marker.ts:87-113 forwards only the separate recordRemote argument to writeHaltRecord, and conductor.ts:2303-2315 passes this.events as marker telemetry but no recordRemote, so publishHaltRecord (halt-record.ts:177-199) calls executeRemoteGit with events undefined; the resolution watcher is built at daemon-cli.ts:1906-1910 without recordRemote, so daemon-deps.ts:592 supersedeHaltRecord has no emitter either — conforming drift under approved ADR D9. Close the class from one source: default the record-publication emitter to the marker emitter inside writeHaltMarker, which also covers the sibling callers that already pass events (task-progress.ts:584, rebase.ts:647/664/747, daemon-runner.ts:813, self-host/gate-halt.ts:42, self-host/build-auth-preflight.ts:59, provider-lifecycle.ts:206, auto-park-halt.ts:76, conductor.ts:6132); task-cli.ts:349 (skill-directed plan-gap HALT, S2.3 in-session CLI parity) passes no emitter and gets the existing startOperatorEventSpine. Found-and-excluded: rewind.ts:55 restoreHalt re-writes an already-published HALT during an operator rewind and is not cited by the review. Examined Tasks 5, 8, 10: none admits halt-marker/daemon-cli/task-cli composition, hence appended tasks; no existing assertion is removed — halt-marker tests that assert record-remote options are updated to include events, preserving Task 13's unconfigured-behavior coverage.
**Governing clause:** adr-2026-09-11-github-operation-ownership D9
**Done when:**
- adr-2026-09-11-github-operation-ownership D9 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab2-6 is complete.

### Task rem-as-built-rem-ab2-7: src/conductor/src/engine/task-cli.ts:349 — wrap the plan-gap writeHaltMarker call with startOperatorEventSpine(projectRoot) (stop in finally) and pass its events so the plan-gap HALT record push emits the fallback on the existing .pipeline/events.jsonl spine; cover with a case in src/conductor/test/engine/github-bot-fallback-wiring.test.ts
**Gate:** as-built
**Rationale:** src/conductor/src/engine/halt-marker.ts:87-113 forwards only the separate recordRemote argument to writeHaltRecord, and conductor.ts:2303-2315 passes this.events as marker telemetry but no recordRemote, so publishHaltRecord (halt-record.ts:177-199) calls executeRemoteGit with events undefined; the resolution watcher is built at daemon-cli.ts:1906-1910 without recordRemote, so daemon-deps.ts:592 supersedeHaltRecord has no emitter either — conforming drift under approved ADR D9. Close the class from one source: default the record-publication emitter to the marker emitter inside writeHaltMarker, which also covers the sibling callers that already pass events (task-progress.ts:584, rebase.ts:647/664/747, daemon-runner.ts:813, self-host/gate-halt.ts:42, self-host/build-auth-preflight.ts:59, provider-lifecycle.ts:206, auto-park-halt.ts:76, conductor.ts:6132); task-cli.ts:349 (skill-directed plan-gap HALT, S2.3 in-session CLI parity) passes no emitter and gets the existing startOperatorEventSpine. Found-and-excluded: rewind.ts:55 restoreHalt re-writes an already-published HALT during an operator rewind and is not cited by the review. Examined Tasks 5, 8, 10: none admits halt-marker/daemon-cli/task-cli composition, hence appended tasks; no existing assertion is removed — halt-marker tests that assert record-remote options are updated to include events, preserving Task 13's unconfigured-behavior coverage.
**Governing clause:** adr-2026-09-11-github-operation-ownership D9
**Done when:**
- adr-2026-09-11-github-operation-ownership D9 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab2-7 is complete.
