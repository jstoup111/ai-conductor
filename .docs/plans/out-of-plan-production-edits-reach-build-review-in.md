...
### Task rem-build-review-derived-provenance-1: src/conductor/test/engine/step-runners.test.ts:3455-3486 — assert the isolated scope grader prompt contains the serialized `"derived":true` provenance for the commit-message-derived widening, so removing src/conductor/src/engine/build-review-inputs.ts:225 makes the test fail
### Task rem-build-review-absent-path-boundary-1: src/conductor/test/engine/plan-scope-containment.test.ts:137-142 and src/conductor/test/engine/per-task-commit-floor.test.ts:350-366 — remove the redundant pure-unit absent-disk fixture and make the runContainmentFloor same-directory-neighbor case declare a nonexistent file, asserting satisfied true with no violations and no accepted widening
### Task rem-build-review-daemon-scope-check-authorization-1: src/conductor/src/execution/daemon-session.ts:68-74 and src/conductor/test/execution/daemon-session-enforcement.test.ts:72-85 — add an approved plan task authorizing `scope-check` for daemon-managed maker sessions and pin that the generated commit-msg hook can invoke it under CONDUCT_DAEMON_SESSION=1; explicitly reseal the plan before retaining e364bb3f
### Task rem-daemon-scope-check-auth: src/conductor/src/execution/daemon-session.ts:68-74 and src/conductor/test/execution/daemon-session-enforcement.test.ts:72-85 — operator-authorized 2026-08-18: daemon-managed maker sessions may invoke scope-check (the generated commit-msg hook at src/conductor/src/engine/git-hook-assets.ts:258 requires it under CONDUCT_DAEMON_SESSION=1); pin with a test that the hook can invoke scope-check in a daemon session, retaining commit e364bb3f
