...
- [ ] Default tests fake every third-party boundary; no real test runner, LLM, GitHub, or network
      call is added
### Task rem-tautology-001: src/conductor/src/engine/step-runners.ts:2063 — make the detached Tautology checkout resolve the source worktree's ignored src/conductor/node_modules dependency installation without adding it to git, and add focused materialization coverage proving a uuid-importing selector loads there
### Task rem-tautology-002: src/conductor/test/engine/build-review-tautology-preflight.test.ts:21-33,274 — replace the exported-helper assertions with scoped-command boundary coverage for RSpec, go test, Vitest, pytest, unstructured nonzero output, and exit zero; require the reverted-production run to execute tests and distinguish direct close-handler mapping
### Task rem-tautology-003: src/conductor/test/engine/build-review-coordinator.test.ts:485-490 — execute the infrastructure-event excerpt selector in the dependency-resolved detached checkout, retain the rubric/lap/reason/excerpt assertion, and require reverted production to execute at least one test and fail because excerpt is absent
### Task rem-tautology-004: src/conductor/test/acceptance/tautology-rubric-never-returns-a-verdict-on-rspec-.acceptance.test.ts:195,248 — execute both story selectors in the dependency-resolved detached checkout and require reverted production to run tests and fail specifically on the missing judged RSpec nonzero-exit verdict and missing signaled-run event excerpt, never on module resolution
### Task rem-scope-001: src/conductor/src/engine/step-runners.ts:182,2104 — delete toTautologyScopedRunResult and its export, inline signal-first, exit-zero, otherwise nonzero-exit mapping in runScopedTautologyCommand's close handler, and remove every test import of the helper in favor of scoped-command boundary coverage
### Task rem-tautology-preflight-001: src/conductor/src/engine/step-runners.ts:2051-2067 and src/conductor/test/engine/build-review-tautology-preflight.test.ts:293,460,513 — correct detached-checkout dependency linking so the uuid-importing selector loads, then require the reverted-production run to execute tests and fail on the nonzero-exit and failureExcerpt assertions rather than module resolution
### Task rem-tautology-coordinator-001: src/conductor/src/engine/step-runners.ts:2051-2067 and src/conductor/test/engine/build-review-coordinator.test.ts:490-499 — make the detached checkout resolve the coordinator test's uuid-dependent import graph and require the reverted coordinator to execute the selector and fail specifically because the emitted infrastructure event lacks excerpt
### Task rem-tautology-acceptance-001: src/conductor/src/engine/step-runners.ts:2051-2067 and src/conductor/test/acceptance/tautology-rubric-never-returns-a-verdict-on-rspec-.acceptance.test.ts:228-286 — remove reliance on ancestor-level dependency fallback, execute both selectors from the checkout-local dependency-resolved tree, and require reverted production to fail on the missing judged RSpec verdict and missing signaled-run excerpt rather than uuid/execa loading
### Task rem-tautology-full-suite-001: src/conductor/src/engine/step-runners.ts:2051-2067 and src/conductor/test/acceptance/full-suite-verification-gate.acceptance.test.ts:15,284-288 — expose execa to the detached checkout and require the selector to execute and distinguish the reverted .ai-conductor/config.yml scoped_command value at assertion level
### Task rem-completeness-materialization-001: src/conductor/test/engine/step-runners.test.ts and src/conductor/src/engine/step-runners.ts:2051-2067 — add focused DefaultStepRunner createCheckout coverage using a temporary project with src/conductor/node_modules present and no ancestor fallback, then assert the checkout-local src/conductor/node_modules symlink resolves a uuid-importing selector
### Task rem-tautology-005: src/conductor/test/engine/build-review-coordinator.test.ts:463-505 — replace the locally constructed legacyEvent JSON round-trip with a coordinateBuildReviewRubrics infrastructure-preflight case whose failureExcerpt is undefined, then assert the emitted build_review_rubric_infrastructure_failure event does not have an excerpt property while retaining the sibling-settlement assertions
### Task rem-completeness-docs-001: docs/reference/configuration.md:839 — replace the Vitest/pytest-only and RSpec-never-verdict claim in the build_review.rubrics row with the shipped contract: exit code zero is green, every nonzero exit is counterfactual RED, and only launch, timeout, and signal are scoped-run infrastructure outcomes
### Task rem-build-review-scope-001: src/conductor/test/acceptance/full-suite-verification-gate.acceptance.test.ts:85,108 — remove CONDUCT_DAEMON_SESSION_UNSAFE_ALLOW from invokeSuite and invokeRealSuite, preserving the existing AI_CONDUCTOR_NO_REAL_EXEC/caller env wiring and the rem-tautology-full-suite-001 detached-checkout scoped_command assertion
