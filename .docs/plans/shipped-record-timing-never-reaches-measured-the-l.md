...
### Task rem-adr-001-b80c33: src/conductor/src/engine/conductor.ts:5644-8615 — funnel every catchable terminal HALT return at :6446, :6472, :6501, :6521, :6569, :6578, :6984, :7062, and :7135 through one conductor-owned exit path that awaits closeOpenExecutions before writing the halt marker, emitting loop_halt, and returning, while leaving the retryable protected-artifact path at :6089 open until its real terminal; src/conductor/test/acceptance/daemon-halts-a-build-that-is-making-forward-progre.acceptance.test.ts:319 — extend the bounded bpCeilingHit fixture with the real EventPersister and assert one interval-bearing terminal per start, terminal-before-loop_halt ordering, and no open-executions timing reason
