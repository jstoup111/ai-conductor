...
### Task rem-scope-copy-equivalence-1: src/conductor/test/engine/step-runners-copy-equivalence.test.ts:133 — fulfill existing Task rem-scope-2 by retaining the byte-identical-repair diagnostic assertion without a source edit, verifying the focused test passes, and recording commit 673756a7a as satisfying evidence
### Task rem-scope-copy-equivalence-2: src/conductor/test/engine/step-runners-copy-equivalence.test.ts:133 — fulfill current Tasks rem-scope-2 and rem-scope-copy-equivalence-1 by retaining the byte-identical-repair diagnostic assertion without a source edit, run the focused test, and record commit 673756a7a as satisfying evidence
### Task rem-tautology-cache-lap-1: src/conductor/test/engine/build-review-cache.test.ts:234 — replace the isolated classifyBuildReviewCacheLookup/projectionDigest assertion with a coordinateBuildReviewRubrics cache-hit regression that supplies the pre-change entry under a current projection with different lap provenance, asserts no fresh dispatch, and asserts the settled result carries the current engine-stamped lapId and snapshotDigest
### Task rem-tautology-reason-pin-1: src/conductor/test/engine/build-review-coordinator.test.ts:611 — remove the runtime expect(reasons).toHaveLength(8) assertion derived from the local pinned literal and retain the exact Record<BuildReviewInfrastructureFailureReason, true> membership pin as the compile-time guard against silent union growth
### Task rem-tautology-v1-artifact-1: src/conductor/test/engine/build-review-artifacts.test.ts:92 — replace the standalone v1 parsing test with a live-stamp boundary regression that first exercises the changed engine-stamped v3 envelope path and then asserts a persisted v1 record still parses under its declared v1 relation grammar
### Task rem-tautology-v2-artifact-1: src/conductor/test/engine/build-review-artifacts.test.ts:101 — replace the standalone v2 parsing test with a live-stamp boundary regression that first exercises the changed engine-stamped v3 envelope path and then asserts a persisted v2 record still parses under its declared v2 relation grammar
### Task rem-scope-relocation-audit-1: src/conductor/src/engine/build-review-coordinator.ts:198 and skills/build-review-tautology/SKILL.md:74 — remove relocationAudit from the provider wire payload and Tautology result contract, then update src/conductor/test/engine/build-review-domain.test.ts:591 and src/conductor/test/engine/build-review-coordinator.test.ts:670 to prove relocationAudit is discarded as an unrecognized top-level key
### Task rem-root-cause-reference-discovery-1: test/check_build_review_rubric_skill_vocabularies.sh:76 — derive the behavioral probe's reference-field set from parser-enforced source rather than a hard-coded referenceFields list, fail closed when extraction is incomplete, and add a secondaryLocus-style fixture that introduces a parser-consumed field while preserving existing fields and proves all four skill contracts reject the unstated grammar
### Task rem-completeness-malformed-dispatch-1: src/conductor/test/engine/build-review-coordinator.test.ts:585 — replace the pre-formed dispatch-failure table with coordinateBuildReviewRubrics dispatches returning not JSON at all, { findings: "none" }, and { findings: [validFinding, malformedFindingWithEmptyAnchorPath] }, then assert the engine-produced diagnostic details respectively name no parseable JSON object, the findings array requirement, and findings[1] with whole-result rejection
### Task rem-tautology-cache-engine-stamp-2: src/conductor/test/engine/build-review-cache.test.ts:236 — retarget the regression so coordinateBuildReviewRubrics first exercises a live provider result through stampBuildReviewDispatchedCandidate and proves the engine-produced v3 envelope, then exercises the pre-change cache entry under different lap provenance and asserts no fresh dispatch plus current-lap rematerialization; remove the same-input projectionDigest comparison at :255-257
### Task rem-tautology-reason-pin-2: src/conductor/test/engine/build-review-coordinator.test.ts:646 — remove the runtime test that enumerates and asserts the locally declared infrastructure-reason literal, and retain the exact Record<BuildReviewInfrastructureFailureReason, true> membership pin only as a compile-time guard so Task 18 proves the union is unchanged without claiming new runtime behavior
### Task rem-root-cause-reference-discovery-2: test/check_build_review_rubric_skill_vocabularies.sh:76-103 — replace each RUBRICS.referenceFields list as the authority with extraction of every reference-bearing field consumed by the corresponding engine parser, and fail closed with a field-specific diagnostic whenever parser-field discovery is incomplete
### Task rem-root-cause-relocation-audit-2: src/conductor/src/engine/build-review-coordinator.ts:193-198 and skills/build-review-tautology/SKILL.md:74 — remove relocationAudit from the provider wire candidate and result contract so only findings reaches engine stamping; update src/conductor/test/engine/build-review-domain.test.ts:591 and src/conductor/test/engine/build-review-coordinator.test.ts:616 to prove relocationAudit is ignored as an unrecognized top-level key and cannot invalidate otherwise valid findings
### Task rem-completeness-reference-fixture-2: test/check_build_review_rubric_skill_vocabularies.sh:134 and :253 — add a fixture whose rootCause parser consumes an optional secondaryLocus content-region reference while existing fields remain unchanged, then assert the four-contract guard names rootCause, secondaryLocus, and its unstated grammar and exits nonzero
