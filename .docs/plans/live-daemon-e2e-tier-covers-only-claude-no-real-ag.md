...
### Task rem-build-review-tautology-1: src/conductor/test/fixtures/live-e2e-providers.test.ts:46-58 — replace the function-shape assertion with behavioral coverage proving the real Codex descriptor's assertCredentialAvailable returns for a present credential, returns for an absent credential when CODEX_HOME/auth.json exists, and throws the named missing-Codex-credential error when neither source exists
### Task rem-build-review-tautology-2: src/conductor/test/structural/smoke-entry-point.test.ts:33 — add mutation-sensitive coverage of src/conductor/scripts/smoke.ts:3 proving the executable entry point forwards process.argv.slice(2), including the config path and selected smoke file, into runSmokeCommand so reverting to process.argv[2] fails
### Task rem-build-review-root-cause-1: src/conductor/test/fixtures/live-e2e-run-body.ts:446-447 and src/conductor/test/fixtures/live-e2e-run-body.test.ts:1 — resolve the provider-native policy with resolveProviderModelPolicy(descriptor.providerKey), pass it as DefaultStepRunner modelPolicy, and prove the Codex live leg dispatches a Codex-native model rather than the Claude default
### Task rem-build-review-completeness-1: src/conductor/test/fixtures/live-e2e-run-body.test.ts:661-675 — replace the count-only post-preflight case with a production-path failed dispatch result (success false, exitCode 1) and assert commandUnresolved and commandUnresolvedName remain undefined and no successful terminal state is reported, preserving the distinction from unresolved-command and diagnostics-only failures
