...
**Files:** src/conductor/src/engine/daemon-tmux.ts, src/conductor/src/engine/daemon-supervisor-cli.ts, src/conductor/test/engine/daemon-tmux.test.ts, src/conductor/test/engine/daemon-supervisor-cli.test.ts, src/conductor/test/engine/canonical-launcher.test.ts, src/conductor/test/engine/daemon-restart-broken-current.test.ts, src/conductor/test/acceptance/daemon-merged-config-967.acceptance.test.ts
**Dependencies:** Task 6
...
**Steps:**
1. Write failing tests: `buildDaemonForegroundCommand` produces a `sh -c` string that runs the quoted launcher `daemon --continuous` as its only command before `rc=$?` and then invokes the quoted launcher `daemon exit-witness --pid $! ...` (capture the child pid via `&` + `wait`), so the daemon is the shell's single child; both `newDetachedSession` and `respawnPane` use it; a real-tmux test on a fixture-owned private socket (`-L`) starts the wrapper around a stub daemon script, `SIGKILL`s the child, and asserts one `daemon_exited` with `signal: SIGKILL` in `.daemon/exit-events.jsonl`, none in `.daemon/events.jsonl`, and the pane still present; a second respawn while the first wrapper is mid-write leaves exactly one line per pid with every line parseable; a bare run (`daemon --continuous` outside tmux) exits without creating `.daemon/exit-events.jsonl`.
2. Verify RED.
...
- A bare `daemon --continuous` run outside tmux exits without creating `.daemon/exit-events.jsonl`, as asserted by the bare-run test.
...
2. Add cases: a task with a trailer but no row is restored `completed` from the trailer (the `[task-seed] restored` log line); a task with no commit and a stale `in_progress` row is reset to `pending`; a backlog scan over a worktree with no `.pipeline/HALT` returns the feature as dispatchable on the next poll.
...
- A trailer-only completed task is restored as `completed` and an uncommitted mid-flight task is reset to `pending`, as asserted by the re-seed cases.
...
| Story 4 negative: Given the daemon is run bare with no tmux session, when it exits, then no witness runs and no `.daemon/exit-events.jsonl` is created by that exit | 9 | "A bare `daemon --continuous` run outside tmux exits without creating `.daemon/exit-events.jsonl`, as asserted by the bare-run test." | diff-local |
