...
### Task rem-prd-audit-rem-fr-s27-1: src/conductor/src/engine/engineer/coherence-validator.ts:176-181 — remove ONLY the `quote.length === 0` clause from the criterion-row parse guard, leaving the `criterion.length === 0` and `!isCriterionVerdict(verdict)` clauses and the sibling guards at :182-185 (empty citedIds) and :189-192 (out-of-vocabulary disposition) untouched, so an empty quote cell parses and is rejected downstream by the existing criterion-naming branch `criterion:quote-empty:<n>` at :641-648. Preserve the coverage plan Task 14 already delivered (an empty quote is rejected) by REWRITING, not deleting, the matched-pair test at src/conductor/test/engine/engineer/coherence-validator.test.ts:170-176 ("rejects a criterion row with an empty required quote..."): it must now assert the row parses and that `checkCriterionCoverage` reports a gap with gapId `criterion:quote-empty:1` whose detail contains the criterion text `Given a widget, when shipped, then it arrives.` verbatim. In the SAME task add the whitespace-only-quote sibling case (a quote cell of `"  "`) asserting the identical gap id, and assert Task 14's third Done when — that the empty-quote message is distinct from the unresolvable-cited-task message `criterion:task-missing:<n>:<id>` emitted at :630-639.
**Gate:** prd-audit
**Rationale:** src/conductor/src/engine/engineer/coherence-validator.ts:176-181 rejects a criterion row whose quote cell is empty at parse time with the artifact-level reason `unparseable-criterion-row`, rendered at :1714-1719 naming only the coherence file path — never the criterion. That preempts the message-bearing branch `criterion:quote-empty:<n>` at :641-648, which does name the criterion verbatim and is therefore dead code for an ordinary empty cell (reachable today only for a whitespace-only quote, which survives `unquote` with length > 0 and collapses in `normalizeWhitespace`). Plan Task 14 is in the active plan and owns exactly this repair — its Done when reads "An empty quote is rejected naming the criterion" and "The two produce distinct messages, asserted by tests on message content" — so this is conforming implementation work inside approved architecture, not a planning omission and not an architectural question. Confidence 95% (verified: both code paths read at the cited lines, and the existing test at src/conductor/test/engine/engineer/coherence-validator.test.ts:170-176 asserts the current parse rejection). Sibling sweep for the same shape — a parse-time guard preempting a criterion-naming branch: the three other criterion parse guards are the empty-`citedIds` guard at :182-185, the out-of-vocabulary-disposition guard at :189-192, and the non-`isCriterionVerdict` clause at :176-181; none has a downstream criterion-naming branch to reach, and S3.5 and S5.3 (both graded PASS) positively require those malformed values to be refused non-waivably, so they are found and deliberately excluded here rather than changed.
**Criterion:** S2.7
**Parent task:** 14
**Done when:**
- S2.7 is satisfied by this task.

### Task rem-prd-audit-rem-fr-s57-1: src/conductor/test/engine/engineer/coherence-validator.test.ts — add the Task 18 enumeration test asserting that every criterion rejection class in src/conductor/src/engine/engineer/coherence-validator.ts:556-658 emits a gap id `evaluateCoherenceWaiver` accepts: drive one row per class through `checkCriterionCoverage` and assert the exact ids `criterion:omitted:<n>`, `criterion:invented:<n>`, `criterion:duplicate:<n>`, `criterion:verdict:<n>`, `criterion:disposition-missing:<n>`, `criterion:disposition-negative:<n>`, `criterion:task-missing:<n>:<id>`, `criterion:quote-empty:<n>` (reachable after rem-fr-s27-1) and `criterion:quote-ungrounded:<n>`, then feed that id set to `evaluateCoherenceWaiver` (src/conductor/src/engine/engineer/coherence-waiver.ts:96) and assert a fresh waiver naming them clears the gate. Build the enumeration from a single in-test literal array of the class names so the test fails when a new class is added without registration, rather than from a hand-copied second list.
**Gate:** prd-audit
**Rationale:** S5.7 requires every criterion rejection class this feature introduced to carry a waivable gap id, but src/conductor/src/engine/engineer/coherence-validator.ts:168-197 collapses five parse-level classes (wrong cell count, empty criterion, unknown verdict, empty quote, empty cited ids, out-of-vocabulary disposition) into `unparseable-criterion-row`, thrown at :1714-1719 with no gap id and no waiver path, while the nine coverage-layer classes at :556-658 all reach `evaluateCoherenceWaiver` at :1814. Plan Task 18 owns this vocabulary — its Done when reads "A test enumerates every criterion rejection class introduced by this feature and asserts each emits a waivable id" — so the repair has an owner and is conforming implementation work, not an architectural decision. Confidence 95% (verified: read the parse guards, the nine gap-emitting branches, and the waiver call site). Resolution splits along a line the approved specs already draw: `quote-empty` becomes waivable via the S2.7 task above (it already has a criterion-naming gap id), while the remaining parse-level classes stay non-waivable because S3.5 and S5.3 — both graded PASS in this same audit — positively require malformed verdicts and out-of-vocabulary dispositions to be refused rather than defaulted, matching the fabricated-id precedent; Task 18 therefore owns the recorded exception, asserted by test, not a widening of the waiver surface. Sibling sweep: `criterion:stories-unparseable` at :557-568 is the one coverage-layer class that returns `{ok:false}` rather than pushing a gap — it is enumerated in the same test as a deliberate fail-closed class.
**Criterion:** S5.7
**Parent task:** 18
**Done when:**
- S5.7 is satisfied by this task.

### Task rem-prd-audit-rem-fr-s57-2: src/conductor/test/engine/engineer/coherence-validator.test.ts — in the same enumeration test record the deliberate non-waivable exception S5.7 leaves open: assert that the parse-level classes at src/conductor/src/engine/engineer/coherence-validator.ts:168-197 (wrong cell count, empty criterion, unknown verdict, empty cited ids, out-of-vocabulary disposition) return `{ok:false, reason:'unparseable-criterion-row'}` from `parseCoherenceArtifact` and reach the non-waivable throw at :1714-1719, with a test-body comment naming S3.5 and S5.3 as the criteria that require malformed values be refused rather than defaulted. Also assert `criterion:stories-unparseable` (:557-568) remains a fail-closed `{ok:false}` reason and not a waivable gap. Do not add a second waiver mechanism or waiver file format — Task 18's Done when forbids it. Preserves the existing parse-rejection coverage at src/conductor/test/engine/engineer/coherence-validator.test.ts:178-185 and :1755-1761 rather than replacing it.
**Gate:** prd-audit
**Rationale:** S5.7 requires every criterion rejection class this feature introduced to carry a waivable gap id, but src/conductor/src/engine/engineer/coherence-validator.ts:168-197 collapses five parse-level classes (wrong cell count, empty criterion, unknown verdict, empty quote, empty cited ids, out-of-vocabulary disposition) into `unparseable-criterion-row`, thrown at :1714-1719 with no gap id and no waiver path, while the nine coverage-layer classes at :556-658 all reach `evaluateCoherenceWaiver` at :1814. Plan Task 18 owns this vocabulary — its Done when reads "A test enumerates every criterion rejection class introduced by this feature and asserts each emits a waivable id" — so the repair has an owner and is conforming implementation work, not an architectural decision. Confidence 95% (verified: read the parse guards, the nine gap-emitting branches, and the waiver call site). Resolution splits along a line the approved specs already draw: `quote-empty` becomes waivable via the S2.7 task above (it already has a criterion-naming gap id), while the remaining parse-level classes stay non-waivable because S3.5 and S5.3 — both graded PASS in this same audit — positively require malformed verdicts and out-of-vocabulary dispositions to be refused rather than defaulted, matching the fabricated-id precedent; Task 18 therefore owns the recorded exception, asserted by test, not a widening of the waiver surface. Sibling sweep: `criterion:stories-unparseable` at :557-568 is the one coverage-layer class that returns `{ok:false}` rather than pushing a gap — it is enumerated in the same test as a deliberate fail-closed class.
**Criterion:** S5.7
**Parent task:** 18
