...
### Task rem-tautology-refusal-1: src/conductor/test/acceptance/finish-correct-refusal-halts-with-guidance.acceptance.test.ts:64-112 — replace the self-fulfilling FinishPublicationCoordinator fake with createProductionFinishPublicationCoordinator, drive a non-halt authored PR through the real judge_pr_prose path, and restore assertions for one refused provider judgment plus judgment_refused detail rendered into .pipeline/HALT
### Task rem-tautology-unattended-1: src/conductor/test/acceptance/unattended-finish-publication.acceptance.test.ts:728-816 — count author_pr_prose and judge_pr_prose dispatches separately and assert exactly one total provider pass for every healthy convergence row (author=1/judge=0 for placeholder entry; author=0/judge=1 for pre-existing authored prose), restoring the unweakened merge-base bound
### Task rem-tautology-unattended-2: src/conductor/test/acceptance/unattended-finish-publication.acceptance.test.ts:728-816 and src/conductor/test/acceptance/finish-publication-non-advancing-transition.acceptance.test.ts:517 — remove the insensitive preAuthored table row if it still cannot fail when halt-signal/label handling is reverted, retaining its ordinary-authored/no-halt judge_pr_prose contract as a dedicated production-coordinator Task 15 negative-path case
### Task rem-completeness-judgment-1: src/conductor/src/engine/finish-publication-production.ts:146-163,184-190,304-319,356-369 — classify independently observed non-placeholder/non-halt prose as judgment-needed, retain the exact revision produced by this coordinator's author_pr_prose pass as accepted so a healthy placeholder-entry run still spends only its original single provider pass, and continue promoting an exact revision after an accepted judgment
### Task rem-completeness-judgment-2: src/conductor/test/acceptance/finish-publication-non-advancing-transition.acceptance.test.ts:517-538 — restore the Task 15 negative variant through createProductionFinishPublicationCoordinator: ordinary authored prose with no halt title, label, marker, or banner reaches judge_pr_prose exactly once, while empty and unrelated label controls make no halt claim
### Task rem-completeness-judgment-3: src/conductor/test/engine/finish-publication-production.test.ts:770 — restore the real-production-coordinator judgment-outcome table for halt, refused, timed_out, provider_unavailable, undecodable, incompleteness, and raw-placeholder results, asserting each mapped disposition and that neither gh pr ready nor recordFinish runs
