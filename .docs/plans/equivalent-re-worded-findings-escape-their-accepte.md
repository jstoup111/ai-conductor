...
### Task rem-tautology-4: src/conductor/test/engine/build-review-disposition-race.test.ts:41-117,248 — extend the fixture to record a build-review mergeBase and deterministic stale-mirage state, then add separate effective-PASS cases that reach scopeFailDisposition halt and invalidated so removing either guard at conductor.ts:7597 or :7618 fails its case
### Task rem-tautology-5: src/conductor/test/engine/build-review-disposition-race.test.ts:252-267 — seed priorVerdict false with matching tree hash and unchanged resolved-task movement so checkKickbackToBuildEscalation returns halt, then assert effective PASS re-enters build_review and removing the guard at conductor.ts:7640 produces the no-op HALT
### Task rem-tautology-6: src/conductor/test/engine/build-review-disposition-race.test.ts:292-307 — seed priorVerdict true, a non-exhausted cumulative count, and the per-gate count at its limit so escalation is bypassed and execution reaches conductor.ts:7761, then prove removing only that effective-PASS guard causes the unresolved-kickback HALT
### Task rem-rootcause-9: src/conductor/src/engine/build-review-domain.ts:35-64 and src/conductor/src/engine/build-review-finding-identity.ts:54-64 — derive and validate a rootCause locus selector from each immutable changed-file path plus its projected hunk range, and hash that canonical selector so two same-class findings in different hunks of one file cannot share an identity
### Task rem-rootcause-10: src/conductor/test/engine/build-review-domain.test.ts:27-70 and src/conductor/test/engine/build-review-finding-identity.test.ts:190-205 — prove only projected path-plus-hunk rootCause selectors parse, same-path same-class findings at distinct hunks receive distinct IDs, and prose rewording leaves each selector's ID unchanged
### Task rem-completeness-5: src/conductor/test/acceptance/equivalent-re-worded-findings-escape-their-accepte.acceptance.test.ts:158-170 — add a Conductor-level case whose rubric result survives vocabulary-rejection repair, then assert the build_review ledger count and cumulative value remain zero and no kickback event is emitted
### Task rem-tautology-7: src/conductor/test/engine/step-runners.test.ts:4001,4068 — replace the shared concern-kind assertion with assertions that the repair prompt contains the literal bounded-previous-output heading followed by content unique to each initial model output
### Task rem-tautology-8: src/conductor/test/engine/build-review-finding-identity.test.ts:109-116 — normalize every closed-vocabulary member with normalizeBuildReviewFindingVocabularyMember and retain the non-empty and collision assertions so production alias folding is exercised
### Task rem-completeness-7: src/conductor/test/engine/step-runners.test.ts:3572-3596 — read the kickback ledger after the surviving vocabulary rejection and assert the build_review gate count and cumulative value both remain zero
### Task rem-rootcause-12: src/conductor/test/engine/build-review-domain.test.ts and src/conductor/test/engine/build-review-finding-identity.test.ts — RED: per adr-2026-08-18-content-anchored-finding-reference-schema, write failing tests proving rootCause locus parses only as a content-region reference { path, contentHash, display } (coordinate encodings rejected); same-path same-class findings with distinct whitespace-normalized hunk content receive distinct identities; prose rewording and pure line-shifts (same content, moved coordinates) leave each identity unchanged; content change yields a new identity; and an integrity test pins the three-kind reference set and per-rubric anchor bindings
### Task rem-rootcause-13: src/conductor/src/engine/build-review-domain.ts and src/conductor/src/engine/build-review-finding-identity.ts — GREEN: derive and validate the rootCause locus as a content-region reference whose contentHash is sha256 of the projected hunk's whitespace-normalized added+removed line content with display excluded from the identity hash, replacing the retired coordinate selector (revert 8977ba7c7 stands); keep v1/v2 identities readable under allowLegacyV1 and bind new findings under the next contract version
### Task rem-rootcause-9: RETIRED — superseded by Task rem-rootcause-12 per adr-2026-08-18-content-anchored-finding-reference-schema (coordinate hunk-locus identity rejected by operator; architecture_review 337164c13 bars carrying the coordinate steps forward); no work is performed under this id
### Task rem-rootcause-10: RETIRED — superseded by Task rem-rootcause-13 per adr-2026-08-18-content-anchored-finding-reference-schema; no work is performed under this id
### Task rem-rootcause-14: src/conductor/src/engine/build-review-projections.ts and src/conductor/src/engine/build-review-domain.ts — per the 2026-08-18 ADR amendment, capture changedTestTitles { selector, titleText } into the frozen projection at snapshot time from the graded head's declared describe/it title chains, bind v3 tautology changedTest to content-region { path: selector, contentHash: sha256(whitespace-normalized titleText), display: titleText }, flag entries whose titles cannot be statically extracted for selector-path fallback hashing, and add RED-then-GREEN tests proving title rewording changes identity while selector renames with unchanged titles do not; per the ADR's equal-content amendment, this task also admits the optional occurrence ordinal in content-region references (equal-content hunks of one path and duplicate title chains disambiguated by projection-order ordinal, occurrence 0 identical to omitted)

### Task rem-br-tautology-normalization-1: src/conductor/test/engine/build-review-finding-identity.test.ts:21-30,263 — delete the test-local contentHashForHunk helper, feed whitespace-varied projected hunks through deriveChangedFileReferences or deriveBuildReviewRubricProjections, and assert the production-emitted rootCause locus contentHash values and identities are equal
### Task rem-br-scope-reaper-1: src/conductor/test/tmpdir-leak-guard.ts:87,204,245 — remove the committed abandoned-PID root-reaper and read-only-directory cleanup changes, restoring the pre-feature global Vitest temporary-root lifecycle
### Task rem-br-scope-scratch-1: src/conductor/test/tmpdir-leak-guard.ts:137 and src/conductor/test/tmpdir-leak-guard.test.ts:225 — remove the committed Codex runtime scratch-file exemption and its regression case, restoring the pre-feature leak-guard ignore set
### Task rem-br-completeness-dispatch-1: src/conductor/src/engine/step-runners.ts:1933-1934 — derive the dispatched contractVersion from CURRENT_BUILD_REVIEW_RUBRIC_CONTRACT_VERSION and replace the plain-string anchor instruction with wording consistent with the rendered v3 typed-reference schema
### Task rem-br-completeness-dispatch-2: src/conductor/test/engine/step-runners.test.ts:4001 — extend the initial rubric-dispatch prompt test to assert the engine-owned current contract version is rendered and the prompt no longer claims every anchor value is a plain string
