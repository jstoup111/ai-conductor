...
### Task T1 — Config type: `build_progress_halt` block
...
### Task T3 — Sidecar field `lastResolvedCount`
...
### Task T4 — Within-dispatch progress-bypass gate
...
### Task T5 — Absolute attempt-ceiling backstop + distinct reason
...
### Task T6 — Zero-progress path regression lock
...
### Task T7 — Record `lastResolvedCount` at dispatch end
...
### Task T8 — Cross-dispatch progress-gated re-kick eligibility
...
### Task T9 — Per-spec dispatch-ceiling bound + reason
...
### Task T10 — Double-dispatch guard integration
