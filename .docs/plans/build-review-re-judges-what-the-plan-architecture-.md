...
### Task rem-fr8-6: src/conductor/src/engine/conductor.ts:5125 and src/conductor/test/engine/conductor.test.ts — replace the disabled-step reason with `steps.${step.name}.disable: true` and assert that exact setting in skip evidence and the config_skip event, preserving plan task rem-fr8-3's operator-visible skip-reason coverage
### Task rem-fr17-4: src/conductor/src/engine/conductor.ts:3396-3401 and src/conductor/test/engine/conductor-remediation-authority-routing.test.ts — build the remediation hint from appendGaps and regress mixed matched/unmatched FR gaps so BUILD receives only authorized appended work, preserving rem-fr17-3's criterion-binding and metadata coverage
### Task rem-fr18-3: src/conductor/src/engine/conductor.ts:3112-3179,6023,6180,8062,8713,8814,9141, src/conductor/src/engine/kickback-ledger.ts:314-319, and src/conductor/test/prd-audit-kickback.test.ts — centralize authorization immediately before every appendRemediationTasks call, permit non-prd_audit gates zero unbounded additions, record accepted growth by originating gate, and prove no caller bypasses or resets allowance while preserving plan Tasks 22-23's prd_audit lap and cap behavior
### Task rem-fr22-3: src/conductor/src/engine/build-review-dispositions.ts:135-137, src/conductor/src/engine/config.ts:89-96, and src/conductor/test/engine/build-review-dispositions.test.ts — derive retired-disposition compatibility from DEPRECATED_BUILD_REVIEW_RUBRIC_IDS and assert a persisted tautology disposition resolves to `{ ok: true, records: [] }`, preserving plan Task 29's tolerant-reader contract and Task 5's six-id deprecated-key set
### Task rem-fr23-7: src/conductor/src/engine/build-review-disposition.ts:246-263, src/conductor/src/engine/conductor.ts:8694-8746, and src/conductor/test/engine/conductor.test.ts — delete the unreachable completeness/remediation arm and stale human/PLAN prose, narrow BuildReviewFailRoute to build|none, remove the unread rubric/findings parameters, and retain Task 11's current test-quality FAIL-to-BUILD behavior plus Task 30's live scopeContainmentEnforced floor
### Task rem-fr17-5: src/conductor/src/engine/conductor.ts:3088-3145,3394-3421 and src/conductor/test/engine/conductor-remediation-authority-routing.test.ts — compute one admitted gap set containing criterion-bound appends plus authorized non-appending publication/halt gaps; refuse and halt when any routed gap falls outside that allowance, including all-unbound and taskless-unbound cases; render BUILD hints only from admitted gaps, retaining the raw fixes fallback solely for build_stall/build-stall sources and preserving rem-fr17-3/rem-fr17-4 mixed-gap behavior
### Task rem-fr23-8: src/conductor/src/engine/build-review-disposition.ts:237-257 — delete the retired routing banner, BuildReviewFailRoute type, JSDoc, and zero-caller buildReviewFailRoute function while preserving rem-fr23-7's live test-quality FAIL-to-BUILD and scopeContainmentEnforced behavior
### Task rem-adr-one-owner-v1-1: src/conductor/test/acceptance/gate-kickback-counter-resets-every-dispatch-so-no-.acceptance.test.ts:170,763 — reword the two stale prose comments to describe test-quality FAIL re-entering BUILD directly, without changing test bodies or call-site behavior
### Task rem-adr-one-owner-v2-1: src/conductor/src/engine/plan-task-parse.ts:158-175, src/conductor/test/engine/plan-task-parse.test.ts:11,55,65,107, and skills/plan/SKILL.md:124,198-210 — remove parsePlanTaskPreserves, its dead-code tests, the Preserves template line, and the complete Preserves marker authoring section while retaining the live Verify-only and Done when parsers/contracts
### Task rem-adr-one-owner-v3-1: src/conductor/src/engine/artifacts.ts:3945-3953 and src/conductor/test/prd-audit-coverage.test.ts:9,313-380 — delete findFrIdsWithoutRows, remove its import and dead-code test block, and leave the live PRD-presence plus story-criterion coverage machinery unchanged
### Task rem-fr9-1-fabea5: src/conductor/src/engine/conductor.ts:5666-5676,6186-6245,669-712,7649-7685 and src/conductor/test/prd-audit-kickback.test.ts — make the SHIP validation-group join derive OVER_SCOPE satisfaction and routing from the same Task 25 result used by the serial walk; cover within-intent and outside-harmless record/pass (including the S13.4 probe-file finding) plus outside-visible halt, preserving Task 25 S9.1/S9.2/S9.6 behavior and coordinating the matched PLAN_GAP sibling handled by rem-fr14-1
### Task rem-fr14-1-c8d530: src/conductor/src/engine/conductor.ts:5666-5676,592-624,6186-6245,7649-7685 and src/conductor/test/prd-audit-kickback.test.ts — make the SHIP validation-group join derive negative-path PLAN_GAP satisfaction and routing from the same Task 24 result used by the serial walk; add grouped-path regression coverage while preserving Task 24 S11.1 and existing S11.2/S11.3/S11.4 halts, sharing the route-result source established with rem-fr9-1
### Task rem-scope-config-1: .ai-conductor/config.yml:51-106,183-203 — remove the feature-local build_review.enabled, completeness.enabled, scope.enabled, and steps.prd_audit.disable overrides plus their workaround comments, restore the previously authorized four-rubric repository configuration, and preserve amended Plan Task 32's scaffolder and migration deliverables while making no new repository-config migration
### Task rem-scope-fr17-probe-1: src/conductor/test/engine/zzz-audit-probe-fr17.test.ts:1 and src/conductor/test/engine/conductor-remediation-authority-routing.test.ts:172 — compare P1-P11 with the authorized suite, migrate each distinct all-unbound, mixed-gap, taskless, publication, finish, validation-group, lowercase-id, and stale-artifact expectation as a real assertion, then delete the probe while preserving rem-fr17-3/rem-fr17-4/rem-fr17-5 criterion-binding and admitted-gap routing coverage
### Task rem-scope-fr17-probe-2: src/conductor/test/engine/zzz-audit-probe2.test.ts:1 — delete the no-op target-reconstruction probe, retaining the real mixed admitted/unadmitted routing assertions in src/conductor/test/engine/conductor-remediation-authority-routing.test.ts so rem-fr17-3/rem-fr17-4/rem-fr17-5 coverage survives unchanged
### Task rem-scope-fr18-probe-1: src/conductor/test/zz-fr18-probe.test.ts:1 and src/conductor/test/prd-audit-kickback.test.ts:265 — migrate every nonduplicate P1-P4 assertion for zero foreign-gate growth, smuggled publication or sealed-artifact tasks, cumulative task and ratio caps, and recorded foreign additions into deterministic assertions in the authorized suite, then delete the standalone probe while preserving rem-fr18-1/rem-fr18-2/rem-fr18-3 plan-growth and ledger coverage
### Task rem-scope-stale-respawn-1: src/conductor/test/engine/daemon-stale-respawn-e2e.test.ts:9-73,786-869 — revert the rem-fr18-3 tmux portability rewrite to the pre-feature isProcessAlive-based predecessor-death and successor-liveness assertions and original boot command, preserving the stale-respawn Task 17 single-generation coverage while leaving rem-fr18-3 plan-growth tests in src/conductor/test/prd-audit-kickback.test.ts unchanged
### Task rem-scope-suite-override-1: src/conductor/test/setup.ts:24-42 — remove the suite-wide CONDUCT_DAEMON_SESSION_UNSAFE_ALLOW assignment and its explanatory block, preserving the explicit marked-environment refusal and valve assertions in src/conductor/test/execution/daemon-session-enforcement.test.ts and keeping any necessary fixture-specific override local to an already plan-authorized test
