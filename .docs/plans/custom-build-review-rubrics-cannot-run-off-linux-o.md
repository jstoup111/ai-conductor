...
### Task rem-as-built-rem-ab12-1: src/conductor/src/engine/step-runners.ts:2580-2600 — make one post-join settlement boundary for customPolicyLap: after Promise.allSettled (including when either branch rejected, and the no-durable-result exit at :2596), always run finishCustomLapInputDigests() and, when it returns changedInputs, give mutation precedence by settling every member review-input-mutated through the same conversion/event/mechanical-fault routing used at :2700-2725 (extract that conversion into one helper both paths call) and discarding cache writes before returning; only when no input changed rethrow the rejection or return the original failure. RED-first in src/conductor/test/integration/build-review-custom-policy.integration.test.ts: a lap where one member's provider fails while a fixture reviewer modifies a frozen-head file settles review-input-mutated naming the path with no aggregate and mechanicalFaults +1, and a failing lap with unchanged inputs still returns its original failure; Task 7's existing mutation-lap tests unchanged
**Gate:** as-built
**Rationale:** As-built AB-12 REMEDIABLE under approved adr-2026-09-10 D5.3 (architecture unchanged, conforming code drift): step-runners.ts:2581-2582 rethrows a rejected fan-out branch and :2586-2593 calls finishCustomLapInputDigests() but discards its changedInputs and returns the original failure, so provider/artifact-persistence failures (:3397-3403) bypass the review-input-mutated conversion that only runs at :2700-2725; sibling exit at :2596-2599 (no durable result) has the same shape and is included; no existing Done-when admits failure-exit settlement, so one new task (plan growth 5/10 -> 6/10); Task 7 and rem-as-built-rem-ab1-1 mutation-lap assertions are preserved.
**Governing clause:** adr-2026-09-10-portable-build-review-policy D5.3
**Done when:**
- adr-2026-09-10-portable-build-review-policy D5.3 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab12-1 is complete.

### Task rem-as-built-rem-ab13-1: src/conductor/src/engine/build-review-input-integrity.ts:66-110 — add a 'symlink' kind to the BuildReviewInputIntegrityFilesystem lstat result and a readlink member, record each symlink as a digest entry keyed by root kind + relative path whose identity is its link target text (never followed, so escapes and cycles cannot recurse), and have diffBuildReviewInputDigests report symlink add/remove/retarget like a file change; update the injected fake filesystem in src/conductor/test/engine/build-review-input-integrity.test.ts to the same kind union and add RED-first unit tests for symlink added, removed, and retargeted under the installed policy package and frozen head roots plus a self-referential cycle that captures without hanging; Task 5's existing regular-file and evidence-root assertions unchanged
**Gate:** as-built
**Rationale:** As-built AB-13 REMEDIABLE under approved adr-2026-09-10 D5.3: build-review-input-integrity.ts:70-76 classifies a symlink as 'other' and collectRegularFiles at :90-108 returns nothing for it, so a symlink add/remove/retarget under installed policy packages (which build-review-policy-bundle.ts:232-260,277-285 support) leaves equal digests; Task 5's Done-when (regular-file content hashes) does not admit symlink identity, so one new task (growth -> 7/10); the filesystem kind union and the test's injected fake reader are a matched pair and change together.
**Governing clause:** adr-2026-09-10-portable-build-review-policy D5.3
**Done when:**
- adr-2026-09-10-portable-build-review-policy D5.3 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab13-1 is complete.

### Task rem-as-built-rem-ab14-1: src/conductor/src/engine/step-runners.ts:2761-2769 and :3438-3446 — in BOTH read-only-review-unavailable halt branches, publish the current-lap aggregate (via the same publication used at :2802-2823 / :3470-3487, carrying the read-only-review-unavailable infrastructure-failure results) before returning the needs-human refusal, keeping the mechanical-fault counter unchanged; RED-first add a test in src/conductor/test/engine/build-review-cli.test.ts (or build-review-custom-convergence.test.ts) that drives a real read-only-review-unavailable halt through the runner, then runs record-reduced-coverage for that lap against the written .pipeline/build-review.json and asserts the decision is stored, and that a re-run resolves the halted rubric through the stored disposition; Task 14 halt assertions and Task 15 CLI refusal assertions unchanged
**Gate:** as-built
**Rationale:** As-built AB-14 REMEDIABLE under approved adr-2026-08-18 D3.2: every lap deletes .pipeline/build-review.json (step-runners.ts:2374-2378) and both read-only-review-unavailable branches (:2761-2769 mixed lap, :3438-3446 custom-only) return before aggregate publication (:2802-2823, :3470-3487), yet build-review-cli.ts:505-513 and :527-536 require that aggregate, so the Task 15 exemption at :539-542 is unreachable after a real halt; both halt branches are a matched pair and are fixed together; one new task (growth -> 8/10); Task 14's halt assertions (needs-human, counter unchanged) and Task 15's CLI assertions are preserved.
**Governing clause:** adr-2026-08-18-mechanical-rubric-faults-are-their-own-lane D3.2
**Done when:**
- adr-2026-08-18-mechanical-rubric-faults-are-their-own-lane D3.2 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab14-1 is complete.

### Task rem-as-built-rem-ab1-1-2: src/conductor/src/engine/step-runners.ts:922 and :2392-2411 — default readOnlyReviewCapabilityProbe to the production probeReadOnlyReviewCapability (options?.probeReadOnlyReviewCapability ?? probeReadOnlyReviewCapability) so every production DefaultStepRunner (daemon-cli.ts:1357/:1595/:2487/:2576, index.ts:1583) probes a provider absent from the threaded startup capabilities once per run and memoizes it before candidate admission, per plan Task 13 Step 3; rewrite the :2392-2395 comment to state startup results are consumed first and the per-run memoized probe covers providers the startup collectors did not probe (e.g. a built-in peer's provider); keep startup collectors (daemon-cli.ts:525-537, index.ts:447-457) custom-only so Task 10's no-custom-rubric no-probe assertion survives; audit existing step-runner/integration fixtures that relied on an undefined probe meaning unavailable and inject an explicit unavailable probe there so Task 13 and rem-as-built-rem-ab2-2 assertions stay unchanged
**Gate:** as-built
**Rationale:** As-built AB-1 REMEDIABLE (99%, verified): the memoized fallback probe in readOnlyReviewCapabilityFor (step-runners.ts:2396-2411) only runs when StepRunnerOptions.probeReadOnlyReviewCapability is injected (:922), but every production DefaultStepRunner construction (daemon-cli.ts:1357, :1595, :2487, :2576 and index.ts:1583) omits it while both startup collectors (daemon-cli.ts:525-537, index.ts:447-457) probe only custom-entry providers, so a built-in peer on a provider absent from the custom set is skipped at step-runners.ts:3801-3814; plan Task 13 Step 3 ('When it is absent, probe once per run and memoize') governs, and no ADR change is needed. Sibling sweep: fixing the default inside the DefaultStepRunner constructor covers all five production constructions at once rather than editing each call site; startup collectors stay custom-only per Task 10's Done-when (its 'no enabled custom rubric -> no probe spawn' bullet must survive, and the runner fallback only fires inside a custom-policy lap). Preserved coverage: Task 13's four candidate-ladder assertions, Task 10's startup wiring assertions, and rem-as-built-rem-ab2-1/ab2-2's all-unavailable built-in peer settlement stay unchanged; existing fixtures that rely on an absent probe meaning 'unavailable' must inject an explicit unavailable probe instead of relying on the undefined default.
**Parent task:** 13
**Governing clause:** Task 13
**Done when:**
- Task 13 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab1-1-2 is complete.

### Task rem-as-built-rem-ab1-2-2: src/conductor/test/integration/build-review-custom-routing.integration.test.ts — RED-first mixed-provider lap test: custom rubric resolving to Codex with threaded startup capabilities containing only Codex available, and an enabled built-in peer resolving to Claude; construct DefaultStepRunner WITHOUT probeReadOnlyReviewCapability (production shape) but with the probe's process runner observable via a Claude CLI fixture accepting the restricted-mode flags, and assert the Claude capability is probed exactly once for the lap, the built-in peer is invoked with readOnlyReview, and no read-only-review-unavailable result is produced; sibling test with the fixture refusing restricted mode asserts the peer settles read-only-review-unavailable naming Claude (existing rem-as-built-rem-ab2-2 path)
**Gate:** as-built
**Rationale:** As-built AB-1 REMEDIABLE (99%, verified): the memoized fallback probe in readOnlyReviewCapabilityFor (step-runners.ts:2396-2411) only runs when StepRunnerOptions.probeReadOnlyReviewCapability is injected (:922), but every production DefaultStepRunner construction (daemon-cli.ts:1357, :1595, :2487, :2576 and index.ts:1583) omits it while both startup collectors (daemon-cli.ts:525-537, index.ts:447-457) probe only custom-entry providers, so a built-in peer on a provider absent from the custom set is skipped at step-runners.ts:3801-3814; plan Task 13 Step 3 ('When it is absent, probe once per run and memoize') governs, and no ADR change is needed. Sibling sweep: fixing the default inside the DefaultStepRunner constructor covers all five production constructions at once rather than editing each call site; startup collectors stay custom-only per Task 10's Done-when (its 'no enabled custom rubric -> no probe spawn' bullet must survive, and the runner fallback only fires inside a custom-policy lap). Preserved coverage: Task 13's four candidate-ladder assertions, Task 10's startup wiring assertions, and rem-as-built-rem-ab2-1/ab2-2's all-unavailable built-in peer settlement stay unchanged; existing fixtures that rely on an absent probe meaning 'unavailable' must inject an explicit unavailable probe instead of relying on the undefined default.
**Parent task:** 13
**Governing clause:** Task 13
**Done when:**
- Task 13 is satisfied by this task.
- Re-run as-built and confirm task rem-as-built-rem-ab1-2-2 is complete.
