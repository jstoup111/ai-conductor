...
### Task rem-fr1-1: src/conductor/src/engine/step-runners.ts:1912-1917 — add and populate a typed StepRunResult mechanical-fault discriminator instead of making human-facing output load-bearing; extend src/conductor/test/engine/build-review-isolation.test.ts with routing coverage whose diagnostic text does not match the current message
### Task rem-fr4-1: src/conductor/src/engine/build-review-cli.ts:90-96,178 — derive exhausted faults from the persisted mechanical allowance, load reduced-coverage decisions, and render the same effective verdict as the engine; extend src/conductor/test/engine/build-review-cli.test.ts with unexhausted mixed-lap and recorded-reduction cases
...
### Task rem-fr11-1: src/conductor/test/engine/shipped-record.test.ts:419-423 — exercise dispatchShippedRecord through src/conductor/src/engine/shipped-record-cli.ts:206-240, asserting rendered reduced-coverage evidence reaches .docs/shipped/<slug>.md and known-but-unrenderable or missing-current-lap evidence returns nonzero without writing the record
### Task rem-fr12-1: src/conductor/src/engine/step-runners.ts:1968-1973 — scope the success override to infrastructureFailure !== undefined && hasJudgedFinding; add a linked-worktree regression in src/conductor/test/engine/build-review-isolation.test.ts proving a full-coverage judged FAIL retains the pre-change failure and retry behavior
### Task rem-fr12-2: src/conductor/src/engine/conductor.ts:6568-6612 — require the typed current-lap mechanical-fault discriminator before emitting the exhausted-allowance HALT; extend src/conductor/test/engine/build-review-halt-wiring.test.ts with a ceiling-reached ledger followed by an unrelated full-coverage judged failure
### Task rem-fr15-1: src/conductor/test/engine/build-review-isolation.test.ts:237 — replace the non-linked fixture with a real linked-worktree setup and prove a fault-free full-coverage judged FAIL preserves pre-change success=false behavior
...
### Task rem-adr-1: src/conductor/src/engine/build-review-aggregate.ts:233-238,284-288 — replace duplicated reduced-coverage comparisons with matchesBuildReviewReducedCoverageDisposition from src/conductor/src/engine/build-review-dispositions.ts:257-269; move cross-feature, cross-rubric, cross-cause, and malformed-identity assertions into src/conductor/test/engine/build-review-effective.test.ts so they exercise the production reducer
### Task rem-adr-2: src/conductor/src/engine/step-runners.ts:1898-1911 — call bumpMechanicalFaultsInLedger from src/conductor/src/engine/kickback-ledger.ts:229 instead of duplicating read-bump-write; add focused coverage proving exactly one persisted bump per mechanical lap
### Task rem-adr-3: src/conductor/src/engine/artifacts.ts:59 and src/conductor/test/engine/build-review-projections.test.ts:318-349 — remove the production-unreachable renderer wrapper and exercise renderBuildReviewReducedCoverageEvidence directly from build-review-projections.ts, preserving the single renderer already used by src/conductor/src/engine/build-review-effective.ts:20 and src/conductor/src/engine/shipped-record-cli.ts:39
### Task rem-adr-4: src/conductor/src/engine/conductor.ts:6687-6689 — consume the typed StepRunResult mechanical-fault discriminator and eliminate the startsWith routing condition; add src/conductor/test/engine/build-review-halt-wiring.test.ts coverage proving D1 routing is independent of human-facing output text
...
### Task rem-fr12-1-439907: src/conductor/src/engine/step-runners.ts:1969-1978 — scope the success override to infrastructureFailure !== undefined && hasJudgedFinding; add a linked-worktree regression in src/conductor/test/engine/build-review-isolation.test.ts proving a fault-free full-coverage judged FAIL remains success=false
