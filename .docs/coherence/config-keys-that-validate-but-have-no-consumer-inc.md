...
| story | story-5 | task-9, task-10 | covered | Registry module + red-case coverage test |
...
| criterion | Story 5 negative: Given a key added to a validator accepted set with no registry declaration, when the coverage test runs, then it fails naming the undeclared key | task-10 | covered | a key in the sets with no declaration fails naming the key | diff-local |
