...
```text
test_suite infrastructure failure (timeout): warning: core.fsyncObjectFiles is deprecated; use core.fsync instead
...
...[output truncated]...
herits resolved rubric policy | inherits resolved rubric policy | inherits resolved rubric policy | Judges whether criterion-bound changed tests are insensitive to the behavior they claim to cover; preflight is evidence, never a verdict. |
...
Run `bin/generate-model-table` to regenerate the table.
