...
- **Data models:** one, additive and backward-tolerant. `KickbackGateEntry` gains a bounded
  per-site repetition tally alongside `cumulative`. The precedent is exact: `adr-2026-08-12` D1
  ...
  alternative requires. Bounding matters: the tally is per-site and sites are unbounded in
  principle, so the entry needs an explicit cap and eviction rule or the ledger grows without limit.
...
- **Blast radius:** three engine modules (`kickback-ledger.ts`, `conductor.ts`'s FAIL block, a new
  ...
  moderate; the reasoning radius is not, and it concentrates in one place: **the threshold value**.
  It cannot be calibrated. The largest genuine sample on disk is two fresh rubric judgements — every
  other `lap-*` directory is a cache re-stamp per `adr-2026-08-13` D7 — so the number is a
  judgement, exactly as `adr-2026-08-12` recorded its cap of 5 at "confidence 70%, basis: inferred".
  Too low and it halts converging features, which that ADR names as "the expensive failure
  direction"; too high and it never fires before the cumulative cap it is meant to pre-empt, making
  the whole feature inert. That is genuine architectural work and needs an ADR, not a plan-task
  footnote.
...
ADRs constrain, it introduces a threshold that no available data can calibrate, and it must answer
`adr-2026-08-16`'s explicit rejection of path-level collapse by arguing the consequence direction
differs on the halting side. Tiering this S would skip the conflict sweep and the architecture
review — and the sweep is what caught that the first measurement was counting cache re-stamps and
the first key was a field the engine never verifies.
