...
| `trim-skill-frontmatter-descriptions-and-de-accrete` | 2026-08-07 | operator (James Stoup) | Too small to justify a plan. The frontmatter half ships as a hotfix instead; the `writing-system-tests` `## Process` re-numbering (its Tasks 4–6) is dropped. The plan was also unrunnable as authored — its eight tasks use `## Task N — …`, but the build predicate requires `### Task <id>`, so none of them parsed |
| `the-engine-cannot-detect-its-own-spinning-operator` | 2026-08-20 | operator (James Stoup) | **Not implemented.** #1718 covers the solution. Task 1 re-derived the threshold over the current 20-feature corpus and halted: no threshold in 3–6 separates spin from healthy. `rubric-cache-identity` shipped clean with 6 per-rubric failures; `2026-08-13-implementation-drifts` cap-terminated at 3. The key counts how often a rubric failed, not whether it failed on the same thing — `adr-2026-08-17` D2 chose the rubric enum precisely to avoid finding identity, which is why it cannot separate frontier expansion from spin. As specified it would terminal-HALT the converging features #1718 exists to protect. Issue moved to v1.1; see #1652 for the full evidence. |
