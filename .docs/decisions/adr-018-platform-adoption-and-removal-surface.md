...
### Option A: A `conduct memory` subcommand group: `adopt <provider>`, `remove`, `status`
- **How:** `adopt <provider>` performs, idempotently: (1) verify/install the provider plugin; (2)
  ...
  `memory_provider: <provider>` to `.ai-conductor/config.yml` without disturbing other keys. `remove`
...
- **Pros:** One deliberate action per FR-6; reuses the `claude mcp add`/`get` idempotency precedent;
  config write is a targeted key-set (no clobber); symmetric adopt/remove; default needs none (FR-8).
...
Adopt **Option A**: a **`conduct memory` subcommand group** — `adopt <provider>`, `remove`, `status`.

- **Adopt is idempotent and non-clobbering (FR-6):** each sub-step checks-then-acts —
  `claude mcp get` before `claude mcp add`; targeted key-set for `memory_provider` leaving all other
  config intact; re-running `adopt` on an already-adopted provider is a clean no-op. An interrupted
  adopt re-runs to completion with no corrupt/partial state (each step is independently re-entrant).
...
- **Default needs nothing (FR-8):** `local` is the implicit provider; no adopt step, no service, no
  credentials.
...
### Positive
- Turnkey, idempotent adopt/remove per FR-6/FR-7; default untouched per FR-8.
...
### Follow-up Actions
- [ ] Implement `conduct memory adopt|remove|status`, each step check-then-act (idempotent, re-entrant).
...
- [ ] Negative-path coverage: re-adopt no-op, interrupted-adopt re-run, adopt leaves other config
