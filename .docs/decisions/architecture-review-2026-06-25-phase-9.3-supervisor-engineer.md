# Architecture Review: Phase 9.3 — Supervisor / Engineer (capstone)

**Date:** 2026-06-25
**Tier:** Large (full review — all sections)
**Stories reviewed:** `.docs/stories/phase-9.3-supervisor-engineer.md` (12, FR-1..FR-12)
**Inputs:** PRD `2026-06-25-phase-9.3-supervisor-engineer.md`; diagrams
`.docs/architecture/2026-06-25-phase-9.3-supervisor-engineer.md`; conflict report
`.docs/conflicts/2026-06-25-phase-9.3-supervisor-engineer.md`; ADR-003 (registry), adr-002 (store),
`supervisor-engineer-followup.md` §9.3; `registry.ts`/`engineer-store.ts` reader contracts.
...
| FR-5 flywheel read | ✓ store reader | runtime EngineerStoreReader impl | store | med (selection) |
...
No new external dependencies. Runtime `RegistryReader`/`EngineerStoreReader` implementations are new
(9.2/9.1 shipped types only — expected; the engineer owns the read side). All cross-repo/git work is
...
- **Departure from `supervisor-engineer-followup.md` §9.3** (broker governor + supervisor-spawns-workers)
...
- **No ad-hoc rebase** (memory `feedback_no_adhoc_rebase`): the engineer authors specs only; it runs no
...
| Semantic types | `(project, feature)` ledger key and `EngineerSignal` reuse 9.1 types; rates use the shared 9.1 metric. |
...
| A code path lets engineer → build / auto-merge | Security | Low | **High** | ADR-005 structural test: engineer imports neither build/pipeline nor merge entry points |
...
| FR-12 trend contaminated by non-engineer work | Knowledge | Medium | Medium | ADR-006 authored-keys intersection excludes non-engineer signals |
...
- **adr-004** — Engineer authoring model + cross-repo isolation (DS-1, DS-5). *Cross-repo safety
...
- **adr-006** — Flywheel lesson selection + engineer-planned provenance (DS-3, DS-6). *Bounded
...
2. **Structural non-autonomy test exists and passes** — asserts the engineer module imports/invokes
...
3. **Authoring runs no fetch/rebase/pull in the target repo** (memory `feedback_no_adhoc_rebase`);
   impl agents are instructed accordingly. Evaluator checks no rebase machinery is invoked by the engineer.
