# ADR 004: Brain authoring model + cross-repo isolation
...
The brain must author a spec (brainstorm→stories→plan) **in a different project's repo** than the
...
The brain resolves the target repo's canonical path from the registry and spawns the
...
- **Pros:** isolation is enforced by the OS process boundary — there is no in-process global state
  to bleed; if the path is wrong the subprocess fails in *that* dir, never the brain's; reuses the
...
DECIDE runs in the brain process; every skill/step threads an explicit repo argument.
...
- **Cons:** every step must thread + honor the repo arg; **one missed default → writes to the
  brain's own cwd** — precisely the recurring Phase-9 bug, re-introduced by construction.

## Decision

**Adopt Option A — cwd-isolated subprocess authoring.** The brain resolves the target's
...
- **Target resolution:** look up the `ProjectRecord` by the confirmed project; use its canonical
  `path`. If the path is **missing/stale on disk**, the brain **errors before any write** — there is
  **no cwd fallback** (the explicit anti-pattern from FR-11's negative path).
- **Isolation:** authoring runs with the target repo as the subprocess cwd; the brain never `cd`s its
  own process. A test asserts that authoring for project A leaves sibling repo B **and the brain's
...
- [ ] Tests: author A → assert B and brain repo untouched; stale path → pre-write error.
