...
- **A-5 (Escaped test-isolation leak) — RESOLVED this session:** several engineer tests drive the real handoff `ensureRunning` without injecting a launch, so each suite run spawned a real `tmux new-session -d 'conduct-ts daemon --continuous'` daemon that outlived the tmpdir (the green suite hid it — same class as H-1: tests pass while reality differs). Severity: medium (CI/local resource leak). Fixed at the source with an auto-launch kill-switch env (`AI_CONDUCTOR_NO_DAEMON_AUTOLAUNCH`, set globally in `test/setup.ts`) that suppresses only the default non-injected launch — robust against future tests, no per-file whack-a-mole. Discovered during cleanup, not by any gate → process gap: no gate checks for leaked OS/process side-effects after a suite run.
