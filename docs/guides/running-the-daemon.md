...
feature can merge cleanly with the current base. A clean prospective merge records
`rebase_mergeable_skip` and continues to `finish` without rewriting feature history or reopening
downstream gate evidence. The skip line, event, and gate verdict name the exact ref, its sha, and
whether it came from origin or a local branch — for example, `rebase skipped — cleanly mergeable
with origin/main@c6839018bf47 (remote)`.
