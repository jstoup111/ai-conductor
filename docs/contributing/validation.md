...
| 24 | `test/check_build_review_rubric_skill_vocabularies.sh` extracts the closed member set for each of the four rubric contracts from `BUILD_REVIEW_FINDING_VOCABULARIES` and its corresponding `skills/build-review-*/SKILL.md` declaration, then compares the sets in both directions. | An engine-accepted member is undocumented, a documented member is no longer accepted, or either closed set cannot be extracted. | Update the engine vocabulary and the matching skill contract in the same change; do not widen either set with prose-only aliases. |
