...
The root is owned by the coordinator process. A nested configured Vitest run borrows that root, so its
workers stay contained and its teardown cannot remove the outer suite's live transform cache.
