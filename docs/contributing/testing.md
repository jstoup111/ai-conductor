...
real `bin/setup`, which may install dependencies. The reusable live-daemon workflow is advisory when
run normally; callers set `require_credentials: true` to fail if `CLAUDE_CODE_OAUTH_TOKEN` is absent.
