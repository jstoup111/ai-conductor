...
| `PostToolUse` | `Edit\|Write` | `lint-after-edit.sh`, `spec-coverage-check.sh`, `diagram-coverage-check.sh` | 30, 10, 10 |
...
| `lint-after-edit.sh` | `PostToolUse` / `Edit\|Write` | Runs `bundle exec standardrb --no-fix` on edited `*.rb` when `bundle` is on PATH. | No — always exits 0 |
