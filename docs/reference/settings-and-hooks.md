...
| `post-commit-derive-feedback.sh` | `PostToolUse` / `Bash` | Despite the name, not a git post-commit hook. Invokes `ai-conductor derive-feedback --sha <sha>` for fast advisory feedback on a commit; falls back to a bash `Task:` trailer match. Never writes `task-status.json`. | No — exits 0 on every path |
