...
| `post-commit-derive-feedback.sh` | `PostToolUse` / `Bash` | Despite the name, not a git post-commit hook. Returns immediately unless the executed Bash command could create a commit and `HEAD` is newly created; an absent, malformed, or command-less payload falls back to evaluating `HEAD`. It then invokes `ai-conductor derive-feedback --sha <sha>` for fast advisory feedback and falls back to a bash `Task:` trailer match. Never writes `task-status.json`. | No — exits 0 on every path |
