...
| `build-review.json` | `{ verdict: 'PASS'\|'FAIL', reasons?, findings?, rubric: { tautology, scope, rootCause, completeness, wiring }, codeStamp? }`. A false wiring item requires non-empty `findings.wiring`; a missing or malformed item fails closed. | `build_review` step |
