...
| `--report` | boolean | `false` | — | Read-only. Renders `.pipeline/events.jsonl` as step durations, retry hotspots, token spend, and kickbacks by source gate, then exits 0. A report error exits 1. No provider session. |
