...
| --- | --- | --- | --- |
| `capabilities` | `engineer capabilities` | Prints `{schemaVersion:1, engineerLifecycleEventsV1:true}`. Consumers must require this exact capability before relying on the Engineer event, replay, marker, and handoff contract. | 0; 1 on unknown flag |
| `run-create` | `engineer run-create --repo-root <path> --idea "<text>" [--correlation-id <opaque>] [--attempt-key <opaque>]` | Reserves one durable Engineer attempt and prints its compact snapshot. The same repository, correlation id, and attempt key is idempotent. A new attempt key is accepted only after the prior correlated run is terminal. Omitted correlation and attempt keys are engine-minted. | 0; 1 on collision, a concurrent live attempt, invalid input, or unknown flag |
...
| `run-fail` | `engineer run-fail --run-id <id> --error <text>` | Appends a terminal run failure. A correlated retry uses `run-create` with a new attempt key and receives a new run id, ordinal, predecessor, and revision sequence. | 0; 1 on an invalid or terminal run |
| `projects` | `engineer projects` | Prints the registry as JSON. Read-only. | 0; 1 on unknown flag |
| `worktree` | `engineer worktree --project <name> --idea "<text>" [--source-ref <ref>] [--body <text>] [--engineer-run-id <id>]` | Resolves the project and creates the per-idea git worktree and branch. It validates a supplied run or creates an uncorrelated run, writes `.pipeline/engineer-run.json`, then prints `{kind, engineerRunId, slug, branch, worktreePath, reconcile}`. With `--source-ref` and no `--body`, it loads the persisted claim body; a missing record degrades to no staging. | 0; 1 on project/identity resolution, lifecycle persistence, target resolution, or worktree creation error |
...
| `handoff` | `engineer handoff --project <name> --branch <branch> --worktree <path> [--source-ref <ref>]` | Opens the spec PR with the `spec` label and with `gh` running inside the per-idea worktree, durably finalizes a marked Engineer run, then removes the worktree and prints `{kind:'pr-opened', url}` or `{kind:'local-commit', branch, repoPath, reason}`. A durable finalization failure is reported and retains the worktree for retry. Then starts the target repo's daemon, fire-and-forget. With `--source-ref`, writes back to the ledger, applies the `engineer:handled` label, and mirrors the issue's `priority: <band>` criticality labels onto the spec PR (fail-open: a read or write failure logs one `[pr-criticality]` line and never fails the handoff). On PR-open failure it records branch evidence and keeps the worktree. | 0, including when durable finalization or worktree removal fails after delivery (warned); 1 on project not found, target resolution error, or PR open failure |
...
also exit 1. The capability name is exactly `engineerLifecycleEventsV1`.
...
`engineer_run_created`, `engineer_run_started`, `engineer_routing_selected`,
...
`engineer_run_cancelled`, `engineer_run_failed`, and `engineer_run_settled`.
