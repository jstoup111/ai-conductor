...
| `unclaim` | `engineer unclaim <sourceRef>` | Single-entry maintenance: returns a `claimed` ledger entry to pending while preserving its original capture time, so it can be claimed again. Missing or non-claimed entries report a non-error result and are left unchanged. | 0 for handled, absent, or non-claimed entries; 1 on unknown flag |
| `requeue` | `engineer requeue --stale [--older-than <dur>]` | Bulk maintenance: returns stale claimed entries to pending. Without `--older-than`, it uses `stale_claim_window_hours` (24 hours by default); the flag supplies a one-run duration override. Entries whose source issue is confirmed closed are removed instead; unconfirmed liveness failures are reported without removal. | 0 after the sweep; 1 on unknown flag |
...
| `finalize-changelog-pr` | `conduct-ts finalize-changelog-pr --pr-url <url>` | Replaces `{{IMPLEMENTATION_PR}}` tokens in `CHANGELOG.md` with a link. The URL must be strictly canonical `https://github.com/<owner>/<repo>/pull/<n>`. Zero tokens is a no-op. With several tokens it compares against `CHANGELOG.md` at the branch's merge-base and substitutes every token-bearing line this branch introduced — a PR shipping several entries finalizes them all — while a token already present at the merge-base is left untouched (no-op when none are new). If the merge-base file cannot be resolved, several tokens are refused rather than risk misattributing another PR's entry. | 0, including the no-op; 1 on a malformed URL, several tokens with an unresolvable merge-base, or a usage error |
