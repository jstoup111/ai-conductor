...
| `build_review` | an incomplete build — built-in opt-in rubrics (`testQuality` and `security`, both off by default) plus opt-in installed custom rubrics, judged from frozen review input rather than self-reports; an empty rubric set is a PASS with nothing dispatched. See [`build_review.custom_rubrics`](../reference/configuration.md#build_reviewcustom_rubrics). |
...
`security` (both off by default) and opt-in installed custom rubrics; see [`build_review.custom_rubrics`](../reference/configuration.md#build_reviewcustom_rubrics). A `testQuality` finding — a test that could pass against a stub of the behavior it claims
