...
describe('plan Wired-into anchor gate at land time (#1190 machinery, wired to a gate)', () => {
  const gh: GhRunner = async () => ({ stdout: 'bob\n' });

  /** A plan whose single task declares one `**Wired-into:**` anchor. */
  const planWithAnchor = (anchor: string) =>
    `${PLAN_WITH_DEPS}\n### Task 3: Wire the tail\n\n**Story:** Story 1\n**Files:**\n- src/tail.ts\n**Wired-into:** ${anchor}\n`;
  ...
  it('refuses a plan whose anchor names a file that does not exist, naming the task and the anchor', async () => {
    const dir = await seedValidWorktree();
    const headBefore = await git(['rev-parse', 'HEAD'], dir);
    await writeFile(
      join(dir, '.docs', 'plans', 'dep-bump.md'),
      planWithAnchor('src/conductor/src/cli.ts#commandTable'),
    );
    ...
    expect(caught).not.toBeNull();
    expect(caught!.message).toMatch(/Wired-into/i);
    expect(caught!.message).toContain('Task 3');
    expect(caught!.message).toContain('src/conductor/src/cli.ts#commandTable');
    // Nothing was committed: the gate refuses before any write.
    expect(await git(['rev-parse', 'HEAD'], dir)).toBe(headBefore);
  });

  /** Commit a source file on the base branch so the worktree inherits it tracked and clean. */
  async function commitSourceOnBase(path: string, content: string): Promise<void> {
    await mkdir(join(repoPath, 'src'), { recursive: true });
    await writeFile(join(repoPath, path), content);
    await git(['add', path]);
    await git(['commit', '-m', `seed ${path}`]);
  }

  it('refuses an anchor whose file exists but carries no reference to the declared symbol', async () => {
    await commitSourceOnBase('src/host.ts', 'export const unrelated = 1;\n');
    const dir = await seedValidWorktree();
    await writeFile(join(dir, '.docs', 'plans', 'dep-bump.md'), planWithAnchor('src/host.ts#runStep'));

    await expect(
      landSpec(target(), 'dep bump', dir, undefined, { ownerConfig: {}, gh }),
    ).rejects.toThrow(/runStep/);
  });

  for (const tier of ['S', 'M', 'L'] as const) {
    it(`enforces the anchor gate at tier ${tier} — unlike the coherence gate, no tier is exempt`, async () => {
      const dir = await seedTierWorktree(tier);
      await writeFile(
        join(dir, '.docs', 'plans', 'dep-bump.md'),
        planWithAnchor('src/does-not-exist.ts#Ghost'),
      );

      await expect(
        landSpec(target(), 'dep bump', dir, undefined, { ownerConfig: {}, gh }),
      ).rejects.toThrow(/Wired-into/i);
    });
  }

  it('lands a plan whose anchor resolves to a real symbol in a real file', async () => {
    await commitSourceOnBase(
      'src/host.ts',
      'import { tail } from "./tail.js";\nexport const boot = () => tail();\n',
    );
    const dir = await seedValidWorktree();
    await writeFile(join(dir, '.docs', 'plans', 'dep-bump.md'), planWithAnchor('src/host.ts#tail'));

    const result = await landSpec(target(), 'dep bump', dir, undefined, { ownerConfig: {}, gh });
    expect(result.branch).toBeTruthy();
  });

  it('lands a plan declaring the `none (no new production surface)` waiver form', async () => {
    const dir = await seedValidWorktree();
    await writeFile(
      join(dir, '.docs', 'plans', 'dep-bump.md'),
      planWithAnchor('none (no new production surface)'),
    );
  ...
  it('refuses a self-referential anchor naming a symbol in the very file the task creates', async () => {
    // The `build-post-task-tail-telemetry` failure verbatim: Task 9 declared
    // `closeout-tail.ts#CloseoutEventTail` — a symbol in the file that task
    // creates — which asserts no downstream wiring at all. At DECIDE time the
    // file does not exist yet, so the gate refuses instead of letting BUILD
    // discover it and rewrite the approved plan.
    const dir = await seedValidWorktree();
    await writeFile(
      join(dir, '.docs', 'plans', 'dep-bump.md'),
      `${PLAN_WITH_DEPS}\n### Task 9: Add the closeout tail\n\n**Story:** Story 1\n**Files:**\n- src/closeout-tail.ts\n**Wired-into:** src/closeout-tail.ts#CloseoutEventTail\n`,
    );

    await expect(
      landSpec(target(), 'dep bump', dir, undefined, { ownerConfig: {}, gh }),
    ).rejects.toThrow(/closeout-tail\.ts#CloseoutEventTail/);
  });

  it('refuses an anchor path escaping the worktree instead of reading outside it', async () => {
    const dir = await seedValidWorktree();
    await writeFile(
      join(dir, '.docs', 'plans', 'dep-bump.md'),
      planWithAnchor('../../../etc/passwd#root'),
    );
  ...
  it('keeps the protected-target gate ahead of the anchor gate', async () => {
    const dir = await seedValidWorktree();
    await writeFile(
      join(dir, '.docs', 'plans', 'dep-bump.md'),
      `${PLAN_WITH_DEPS}\n### Task 14: Amend another feature's accepted story\n\n**Files:**\n- .docs/stories/other-feature.md\n**Wired-into:** src/does-not-exist.ts#Ghost\n`,
    );

    await expect(
      landSpec(target(), 'dep bump', dir, undefined, { ownerConfig: {}, gh }),
    ).rejects.toThrow(/sealed artifacts owned by another feature/);
  });
});
