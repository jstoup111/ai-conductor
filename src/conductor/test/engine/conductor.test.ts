...
describe('engine/conductor', () => {
  ...
  describe('daemon finish/as-built remediation', () => {
    ...
    it('retries finish once with the recording command when finish-choice is missing', async () => {
      await seedShipTail();
      let finishCalls = 0;
      const runner: StepRunner = {
        run: vi.fn(async (step: StepName) => {
      ...
      const finishDispatches = vi
        .mocked(runner.run)
        .mock.calls.filter(([step]) => step === 'finish');
      expect(finishDispatches).toHaveLength(2);
      const secondFinishOptions = finishDispatches[1]?.[2] as StepRunOptions | undefined;
      expect(secondFinishOptions?.retryReason).toContain('conduct-ts finish-record');
      expect(secondFinishOptions?.retryReason).toContain(
        `--pipeline-dir ${join(dir, '.pipeline')}`,
      );
      expect(vi.mocked(runner.run).mock.calls.map(([step]) => step)).not.toContain('remediate');
      expect(vi.mocked(runner.run).mock.calls.map(([step]) => step)).not.toContain('build');
      const result = await readState(statePath);
      expect(result.ok && result.value.finish).toBe('done');
    });

    it('routes to remediation after the one free finish-recording retry is also missing', async () => {
      await seedShipTail();
      const calls: StepName[] = [];
      const runner: StepRunner = {
        run: vi.fn(async (step: StepName) => {
              ...
              dispositions: [
                {
                  ...
                  disposition: 'halt',
                  category: 'architectural-clarity',
                  rationale: 'finish completion evidence remains absent after its recording retry',
                  tasks: [],
                },
              ],
            });
      ...
      expect(calls.filter((step) => step === 'finish')).toHaveLength(2);
      expect(calls.filter((step) => step === 'remediate')).toHaveLength(1);
