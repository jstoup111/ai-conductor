...
// ─────────────────────────────────────────────────────────────────────────────
// Warn-once WAITING wiring (Task 18 / FR-6): localWorkSource forwards the
// `waiting` list from discoverBacklog into a per-instance announcer, so a
// stable wait logs once per run and a changed verdict re-announces.
// ─────────────────────────────────────────────────────────────────────────────

describe('localWorkSource — WAITING announcements are warn-once across discover() passes', () => {
  it('identical waiting verdict across 3 discover() passes logs exactly once', async () => {
    const mod = await load(WS_MOD);
    const localWorkSource = requireFn(mod, 'localWorkSource');
    const log = vi.fn();
    ...
    const waitingItem = {
      slug: 'feat-a',
      sourceRef: 'acme/repo#1',
      verdict: { kind: 'blocked' as const, blockers: [{ repo: 'acme/repo', number: '1' }] },
    };
    ...
    const deps = {
      ...
      markWarned: vi.fn().mockResolvedValue(undefined),
      fastForwardRoot: vi.fn(async () => {}),
      discoverBacklog: vi.fn(async () => ({ items: [], waiting: [waitingItem] })),
    };
    ...
    const waitingLogs = log.mock.calls.filter((c) => String(c[0]).includes('feat-a'));
    expect(waitingLogs).toHaveLength(1);
  });

  it('a changed blocker set re-announces exactly once more', async () => {
    const mod = await load(WS_MOD);
    const localWorkSource = requireFn(mod, 'localWorkSource');
    const log = vi.fn();
        ...
        waiting: [
          {
            slug: 'feat-a',
            sourceRef: 'acme/repo#1',
            verdict: { kind: 'blocked' as const, blockers: [{ repo: 'acme/repo', number: '2' }] },
          },
        ],
      },
    ];
    ...
    const deps = {
      ...
      markWarned: vi.fn().mockResolvedValue(undefined),
      fastForwardRoot: vi.fn(async () => {}),
      discoverBacklog: vi.fn(async () => results[call++]),
    };
    ...
    const waitingLogs = log.mock.calls.filter((c) => String(c[0]).includes('feat-a'));
    expect(waitingLogs).toHaveLength(2);
  });
});
