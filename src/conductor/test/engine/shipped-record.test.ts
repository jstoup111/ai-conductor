...
describe('projectShippedRecordSubstance', () => {
  const fields = {
    slug: 'billing-export',
    specHash: 'abc123',
    pr: 'https://github.com/acme/repo/pull/42',
  ...
  const metrics = {
    ...
    rubricFailureRates: { testQuality: { failures: 1, judged: 2 } },
    skipReasons: {},
  };
  const acceptedRisk = (contentHash = 'sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'): BuildReviewDispositionRecord => ({
    version: 'v1',
    ...
    finding: canonicalizeBuildReviewFindingIdentity({
      ...
      anchor: {
        rubric: 'testQuality',
        locus: {
          path: 'test/engine/shipped-record.test.ts',
          contentHash,
  ...
  const appendEvidence = (body: string) => appendBuildReviewReducedCoverageEvidence(
    appendBuildReviewAcceptedRisk(
      appendBuildReviewMetrics(
        body,
        metrics,
  ...
  it.each([
    {
      name: 'removes a final Cost block',
      body: () => renderShippedRecordWithCost(fields, rollup),
      expected: () => `${renderShippedRecord(fields)}\n`,
    },
    {
      name: 'removes an interior Cost block before Build Review',
      body: () => appendBuildReviewMetrics(renderShippedRecordWithCost(fields, rollup), metrics),
      expected: () => appendBuildReviewMetrics(renderShippedRecord(fields), metrics),
    },
    {
      name: 'removes a final Time block',
      body: () => appendTimingSection(renderShippedRecordWithCost(fields, rollup), {
        state: 'partial', activeMs: 100,
      }),
      expected: () => `${renderShippedRecord(fields)}\n`,
    },
    {
      name: 'removes an interior Time block before Build Review',
      body: () => appendBuildReviewMetrics(
        appendTimingSection(renderShippedRecordWithCost(fields, rollup), {
          state: 'partial', activeMs: 100,
        }),
        metrics,
      ),
    ...
    {
      name: 'leaves an absent Cost and Time block byte-for-byte unchanged',
      body: () => renderShippedRecord(fields),
      expected: () => renderShippedRecord(fields),
    },
    {
      name: 'preserves frontmatter and following Build Review and evidence sections byte-for-byte',
      body: () => appendEvidence(appendTimingSection(renderShippedRecordWithCost(fields, rollup), {
        state: 'measured', activeMs: 100, providerActiveMs: 40, noProviderActiveMs: 60,
      })),
      expected: () => appendEvidence(renderShippedRecord(fields)),
    },
    {
      name: 'preserves indented and inline Cost headings',
      body: () => `${renderShippedRecord(fields)}\n  ## Cost\nvalue: literal\ntext ## Time remains\n`,
      expected: () => `${renderShippedRecord(fields)}\n  ## Cost\nvalue: literal\ntext ## Time remains\n`,
    },
  ])('$name', ({ body, expected }) => {
    expect(projectShippedRecordSubstance(body())).toBe(expected());
  });

  it.each([
    {
      name: 'Cost values',
      left: () => renderShippedRecordWithCost(fields, rollup),
      right: () => renderShippedRecordWithCost(fields, { ...rollup, dispatches: 2 }),
      equal: true,
    },
    {
      name: 'Time values',
      left: () => appendTimingSection(renderShippedRecordWithCost(fields, rollup), { state: 'partial', activeMs: 100 }),
      right: () => appendTimingSection(renderShippedRecordWithCost(fields, rollup), { state: 'partial', activeMs: 200 }),
      equal: true,
    },
    {
      name: 'frontmatter PR',
      left: () => renderShippedRecordWithCost(fields, rollup),
      right: () => renderShippedRecordWithCost({ ...fields, pr: 'https://github.com/acme/repo/pull/43' }, rollup),
      equal: false,
    },
    {
      name: 'frontmatter spec hash',
      left: () => renderShippedRecordWithCost(fields, rollup),
      right: () => renderShippedRecordWithCost({ ...fields, specHash: 'def456' }, rollup),
      equal: false,
    },
    {
      name: 'Build Review metrics',
      left: () => appendBuildReviewMetrics(renderShippedRecordWithCost(fields, rollup), metrics),
      right: () => appendBuildReviewMetrics(renderShippedRecordWithCost(fields, rollup), { ...metrics, cacheHits: 2 }),
      equal: false,
    },
    {
      name: 'accepted-risk evidence',
      left: () => appendBuildReviewAcceptedRisk(renderShippedRecordWithCost(fields, rollup), [acceptedRisk()]),
      right: () => appendBuildReviewAcceptedRisk(renderShippedRecordWithCost(fields, rollup), [acceptedRisk('sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb')]),
      equal: false,
    },
  ])('treats changed $name as $equal', ({ left, right, equal }) => {
    expect(projectShippedRecordSubstance(left()) === projectShippedRecordSubstance(right())).toBe(equal);
  });
});
