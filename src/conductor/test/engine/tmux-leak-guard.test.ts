...
async function paneCwdSticky(): Promise<boolean> {
  ...
  try {
    ...
    // `tmux -V` can succeed while this process cannot connect to the tmux
    // server. That host cannot exercise the real-tmux authorization path.
    return false;
  } finally {
...
describe('sweepStaleDaemonSessions — permanent-baseline-blindspot fix', () => {
  ...
  it('real tmux: a session created BEFORE any snapshot is taken (simulating debris left by a ' +
    ...
    try {
      ...
      // The exact test session, not a prior probe, is the authorization input.
