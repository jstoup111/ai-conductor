...
// This ledger is DURABLE — it persists to a JSON file under the brain dir and
...
import {
  recordAuthoredKey,
  readAuthoredKeys,
} from '../../../src/engine/brain/authored-ledger.js';

// Helper: create a fresh temp dir for each test so tests are fully isolated.
// We override the brain dir via AI_CONDUCTOR_BRAIN_DIR env variable.
...
describe('authored-keys ledger', () => {
  ...
  beforeEach(async () => {
    ...
    env = { AI_CONDUCTOR_BRAIN_DIR: tempDir };
  });
  ...
  it('returns [] when the ledger file does not exist (brain dir is empty)', async () => {
