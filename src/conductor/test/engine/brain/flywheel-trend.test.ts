...
//
// Computes the learning-trajectory trend over the features the brain actually
// AUTHORED (store signals ∩ authored-keys ledger). Non-brain signals are
// EXCLUDED; ledger-only keys (no store signal) are ABSENT from the result.
//
// Happy path:
//   - >=2 brain-planned features → per-feature rates + trend direction
//
// Negative paths (each falsifiable):
//   (a) fewer-than-2 brain-planned features → "insufficient-data" sentinel
//       (NOT "improving" / "regressing")
//   (b) non-brain signal excluded: a (project,feature) in the store but NOT in
...
} from '../../../src/engine/brain/flywheel-trend.js';
import { createBrainStoreReader } from '../../../src/engine/brain-store.js';
import type { BrainSignal } from '../../../src/engine/brain-store.js';
import type { AuthoredKey } from '../../../src/engine/brain/authored-ledger.js';

// ─── Fixtures ────────────────────────────────────────────────────────────────

/** Build a minimal BrainSignal. Fields that don't affect rates can be defaults. */
function makeSignal(overrides: Partial<BrainSignal> & { project: string; feature: string }): BrainSignal {
  return {
    schemaVersion: 1,
...
/** Write signals as JSONL lines to the brain dir. */
async function seedSignals(brainDir: string, signals: BrainSignal[]): Promise<void> {
  await mkdir(brainDir, { recursive: true });
  const lines = signals.map((s) => JSON.stringify(s)).join('\n') + '\n';
  await writeFile(join(brainDir, 'signals.jsonl'), lines, 'utf-8');
}
...
describe('computeFlywheelTrend', () => {
  ...
  // ── Happy path: >=2 brain-planned features → trend direction + per-feature rates ──
  ...
  describe('happy path: two features with improving kickback trend', () => {
    ...
    it('returns a FlywheelTrend with direction "improving" when kickback rate decreases', async () => {
      ...
      const reader = createBrainStoreReader({ brainDir: tempDir });
  ...
  // ── Negative (a): fewer than 2 brain-planned features → insufficient-data sentinel ──

  describe('negative (a): fewer than 2 brain-planned features → insufficient-data sentinel', () => {
    ...
    it('returns insufficient-data sentinel when the store is empty (zero signals)', async () => {
      // Zero signals → zero brain-planned features even if ledger is non-empty
  ...
  // ── Negative (b): non-brain signal excluded from trend ──────────────────────

  describe('negative (b): non-brain signal excluded from trend output', () => {
    it('excludes a signal whose (project,feature) is in the store but NOT in the ledger', async () => {
      // Falsifiable: if the intersection check is skipped, the non-brain feature
      // would appear in trend.features — this assertion would then fail.
      const brainSig = makeSignal({
        project: 'proj-B',
        feature: 'feat-brain',
        ts: '2026-06-25T08:00:00.000Z',
        kickbacks: [{ from: 'gate', to: 'step', count: 1 }],
      });
      const nonBrainSig = makeSignal({
        project: 'proj-B',
        feature: 'feat-non-brain', // NOT in ledger
        ts: '2026-06-25T09:00:00.000Z',
        kickbacks: [],
      });
      const brainSig2 = makeSignal({
        project: 'proj-B',
        feature: 'feat-brain-2',
        ts: '2026-06-25T10:00:00.000Z',
        kickbacks: [],
      });

      await seedSignals(tempDir, [brainSig, nonBrainSig, brainSig2]);

      const reader = createBrainStoreReader({ brainDir: tempDir });
      // Ledger contains feat-brain and feat-brain-2 ONLY — feat-non-brain is absent
      const ledger: AuthoredKey[] = [
        { project: 'proj-B', feature: 'feat-brain' },
        { project: 'proj-B', feature: 'feat-brain-2' },
      ];
      ...
      expect(featureNames).toContain('feat-brain');
      expect(featureNames).toContain('feat-brain-2');
    });
  });

  // ── Negative (c): ledger key with zero store signals → absent from trend ────

  describe('negative (c): brain-planned feature with no store signals is absent from trend', () => {
