...
import { governorReport } from '../../../src/engine/brain/governor.js';
import type { GovernorReport } from '../../../src/engine/brain/governor.js';
...
async function seedSignals(signals: BrainSignal[], extraLines?: string[]): Promise<string> {
...
describe('governorReport — read-only guarantee (falsifiable)', () => {
  ...
  it('no new files are created in the brain dir by governorReport', async () => {
    await seedSignals([sig1]);
    const signalsPath = join(tmpDir, 'signals.jsonl');
  ...
  it('governorReport with mutating appendSignal spied on the module — spy never fires', async () => {
    ...
    const brainStoreModule = await import('../../../src/engine/brain-store.js');
    const appendSpy = vi.spyOn(brainStoreModule, 'appendSignal');
