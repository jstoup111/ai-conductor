...
describe('ProviderRuntimeSet', () => {
  ...
  it.each([
    ['success', { success: true, output: 'completed', exitCode: 0 }],
    ['authentication rejection', { success: false, output: 'login rejected', exitCode: 1, authFailure: true }],
    ['ordinary failure', { success: false, output: 'ordinary failure', exitCode: 1 }],
  ] as const)(
    'invokes the actual Codex result once after a probe-failed preflight: %s',
    async (_shape, actualResult) => {
      const probeFailed: AuthenticationReadiness = {
        provider: 'codex',
        source: 'cached-login',
        state: 'probe-failed',
      ...
      const invoke = vi.fn(async () => actualResult);
      const codex = {
        invoke,
        invokeInteractive: vi.fn(async () => {}),
        readiness: vi.fn(async () => probeFailed),
      ...
      const readiness = runtime && runtimes?.readinessFor('codex', probeFailed);
      const invokeRuntime = await loadRuntimeInvoker();
      const result = runtime && await invokeRuntime?.(runtime, {
        prompt: 'continue after inconclusive readiness',
        sessionId: 'probe-failed-session',
        resume: false,
        model: 'gpt-5.6-terra',
      });
      ...
      expect({
        ...
        calls: invoke.mock.calls.length,
        result,
        cachedUnavailability: runtime?.runWideUnavailable,
      }).toEqual({
        readiness: probeFailed,
        calls: 1,
        result: actualResult,
        cachedUnavailability: undefined,
      });
    },
  );
  ...
  it.each([
    ...
    ['unparseable-output', { parserRejection: 'invalid-json' }],
    ['spawn-failure', { errno: 'ENOENT' }],
  ] as const)('preserves the closed probe-failure %s through the selected runtime seam', async (kind, facts) => {
    const probeFailed: AuthenticationReadiness = {
      provider: 'codex', source: 'cached-login', state: 'probe-failed',
      probeFailure: { kind, facts },
    } as AuthenticationReadiness;
    const codex = { ...provider(), readiness: vi.fn(async () => probeFailed) };
    const registry = new PluginRegistry();
    registry.register('llm_provider', 'codex', codex);
    registry.markInitialized();

    const runtimes = (await loadRuntimeSetFactory())?.(registry);
    const readiness = runtimes?.readinessFor('codex', probeFailed);

    expect({ result: await readiness?.(), calls: codex.readiness.mock.calls.length })
      .toEqual({ result: probeFailed, calls: 1 });
  });
