...
// ───────────────────────────────────────────────────────────────────────────
// RED acceptance specs for daemon engineer-signal EMISSION wired into the REAL
// `makeRunFeature` (Phase 9.1, FR-1/FR-5/FR-6/FR-7).
//
// Drives the REAL `makeRunFeature` over a real tmp engineer dir (via
// `$AI_CONDUCTOR_ENGINEER_DIR`) and real tmp worktree/project dirs. The ONLY mock
...
describe('integration/engineer-emission — makeRunFeature emits on daemon completion', () => {
  ...
  it('daemon done → store narrative written, repo .docs/retros/ gets NO new file', async () => {
    ...
    const rec = JSON.parse(lines[0]);
    expect(rec.narrativeRef).toBeTruthy();
    await expect(access(join(engineerDir, rec.narrativeRef))).resolves.toBeUndefined();
