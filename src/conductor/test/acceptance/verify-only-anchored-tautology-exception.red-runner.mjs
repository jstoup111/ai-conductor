...
const target = 'test/acceptance/verify-only-anchored-tautology-exception.acceptance.test.ts';
const reportPath = join(tmpdir(), `verify-only-tautology-red-${process.pid}.json`);
...
try {
  ...
  evidence = {
    ...
    intentRationale:
      'Missing verify-only context in the real assembly, projection, and prompt flow proves #1579 remains unimplemented.',
  };
} catch (error) {
  evidence = {
    executed: 0,
