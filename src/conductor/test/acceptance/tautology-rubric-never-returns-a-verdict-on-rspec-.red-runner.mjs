...
const target = 'test/acceptance/tautology-rubric-never-returns-a-verdict-on-rspec-.acceptance.test.ts';
const reportPath = join(tmpdir(), `tautology-rspec-red-${process.pid}.json`);
...
try {
  ...
  evidence = {
    ...
    intentRationale:
      'The RSpec projection lacks the framework-agnostic nonzero-exit run kind and the signaled run event drops its output, proving the requested verdict and diagnostic behavior are not implemented.',
  };
} catch (error) {
  evidence = {
    executed: 0,
