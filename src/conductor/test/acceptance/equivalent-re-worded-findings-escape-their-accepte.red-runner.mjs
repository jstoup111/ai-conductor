...
const target = 'test/acceptance/equivalent-re-worded-findings-escape-their-accepte.acceptance.test.ts';
const reportPath = join(tmpdir(), `equivalent-finding-red-${process.pid}.json`);
...
try {
  ...
  evidence = {
    ...
    intentRationale:
      'The remediation-refusal HALT proves at least one build_review FAIL exit still decides on the raw aggregate after every finding becomes operator-accepted.',
  };
} catch (error) {
  evidence = {
    executed: 0,
