...
const target = 'test/acceptance/repeated-build-review-semantic-failures-can-churn-.acceptance.test.ts';
const reportPath = join(tmpdir(), `repeated-build-review-red-${process.pid}.json`);
...
try {
  ...
  evidence = {
    ...
    intentRationale:
      'The missing sixth-lap halt and missing diff-derived prompt evidence prove the requested convergence and grading behavior is not implemented.',
  };
} catch (error) {
  evidence = {
    executed: 0,
