  ...
  it('misses Scope alone after production assembly observes a new operator reseal', async () => {
    ...
    const provider: LLMProvider = {
      invoke: vi.fn(async (options) => {
        ...
        return { success: true, output: JSON.stringify({ kind: 'judged', rubric: projection.rubric, lapId: projection.lapId, snapshotDigest: projection.snapshotDigest, contractVersion: 'v2', findings: [], verdict: 'PASS' }), exitCode: 0 };
      }),
      invokeInteractive: vi.fn().mockResolvedValue(undefined),
    };
