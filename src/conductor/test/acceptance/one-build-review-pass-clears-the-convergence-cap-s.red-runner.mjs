...
const target = 'test/acceptance/one-build-review-pass-clears-the-convergence-cap-s.acceptance.test.ts';
const reportPath = join(tmpdir(), `one-build-review-pass-red-${process.pid}.json`);
...
try {
  ...
  evidence = {
    ...
    intentRationale:
      'The second rebuild proves that the intervening build_review PASS incorrectly cleared the cumulative convergence budget instead of reaching the existing halt on the next FAIL.',
  };
} catch (error) {
  evidence = {
    executed: 0,
