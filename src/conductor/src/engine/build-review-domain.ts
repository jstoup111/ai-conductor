...
export function describeBuildReviewJudgedResultRejection(value: unknown, rubric: BuildReviewRubricId, expected: { readonly lapId: string; readonly snapshotDigest: string }, references?: BuildReviewFindingReferenceContext): string { const parsed = parseBuildReviewJudgedResult(value, references); return parsed && parsed.rubric === rubric && parsed.lapId === expected.lapId && parsed.snapshotDigest === expected.snapshotDigest ? 'the result parsed but did not satisfy the judged contract' : `result must match ${renderBuildReviewJudgedResultShape(rubric)} and echo the projection lapId and snapshotDigest`; }
