 ...
 * over a set of BrainSignals. It is distinct from the 429-backoff rate in
 ...
 *
 * Design:
 *   - Reuses `BrainSignal.tokens`, `BrainSignal.kickbacks`, `BrainSignal.halts`,
 *     and `BrainSignal.retryHotspots` — already assembled by `assembleSignal`
...
import type { BrainSignal } from '../brain-store.js';
...
/**
 * Compute aggregate metric rates over a set of BrainSignals.
 *
 * Reuses the pre-aggregated fields on each BrainSignal (assembled via
 * report-renderer.ts helpers in brain-store.ts#assembleSignal) — no
 ...
 */
export function computeSignalRates(signals: BrainSignal[]): SignalRates {
