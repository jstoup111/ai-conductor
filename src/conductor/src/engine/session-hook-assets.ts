...
export const PRE_DISPATCH_HOOK = [
  ...
  '  let removalError = "";',
  '  try { rmSync(currentTaskPath, { force: true }); } catch (err) { removalFailed = true; removalError = String(err && err.message); }',
  '  process.stderr.write(`pre-dispatch-hook: abstain — ${reason}${removalFailed ? `, and stamp removal also failed: ${removalError}` : ""} (dispatch Task: ${id})\\n`);',
  '  process.exit(0);',
  '};',
  '',
  'let locked = false;',
  ...
  'if (!row) {',
  '  const validIds = parsed.tasks.map((t) => t && t.id).filter((value) => value !== undefined);',
  '  process.stderr.write(`PRE dispatch hook: unknown task id "${id}" — not found in task-status.json.\\nValid ids: ${validIds.join(", ")}\\n`);',
  '  process.exit(2);',
  '}',
  'let stampedId;',
  'try { stampedId = readFileSync(currentTaskPath, "utf-8"); } catch {}',
  ...
  '}',
  '// Advisory stamp only: never mutation authorization or completion evidence.',
  'if (stampedId !== undefined && stampedId !== id) {',
  '  try { rmSync(currentTaskPath, { force: true }); } catch {}',
...
export const POST_DISPATCH_HOOK = [
  ...
  'process.stdin.on("data", (chunk) => { data += chunk; });',
  'process.stdin.on("end", () => {',
  '  try { const prompt = JSON.parse(data)?.tool_input?.prompt; if (typeof prompt === "string") process.stdout.write(prompt.split("\\n")[0]); } catch {}',
  '});',
  '\' || true)"',
  'if ! [[ "$LINE1" =~ ^Task:\\ ([A-Za-z0-9._-]+|none)$ ]]; then exit 0; fi',
  'TASK_ID="${BASH_REMATCH[1]}"',
  ...
  'const { join } = require("path");',
  'const id = process.argv[1];',
  'const stamp = join(process.cwd(), ".pipeline", "current-task");',
  'let value;',
  'try { value = readFileSync(stamp, "utf-8"); } catch { process.exit(0); }',
  'if (value !== id) { process.stderr.write(`post-dispatch-hook: task id mismatch: requested "${id}" but stamp contains "${value}" — leaving stamp untouched.\\n`); process.exit(0); }',
  'try { rmSync(stamp, { force: true }); } catch (err) { process.stderr.write("post-dispatch-hook: failed to remove stamp file: " + String(err && err.message) + "\\n"); }',
  '\' "$TASK_ID"',
  'exit 0',
].join('\n');
