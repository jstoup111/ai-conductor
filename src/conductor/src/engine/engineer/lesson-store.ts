...
import type { EngineerStoreReader, EngineerSignal } from '../engineer-store.js';
...
/**
 * Map a EngineerSignal to one or more RetrievedLessons.
 *
 * Mapping strategy:
 ...
 *   metadata → EngineerSignal scalar fields + outcome for downstream filtering
 ...
 */
function signalToLessons(sig: EngineerSignal): RetrievedLesson[] {
  ...
  // (used by the engineer acceptance test fixtures) or an `evidence` field
  ...
  for (let i = 0; i < sig.kickbacks.length; i++) {
    ...
    // KickbackEntry TypeScript type (e.g. engineer acceptance fixtures use
 ...
 */
function keywordMatches(sig: EngineerSignal, queryText: string): boolean {
...
/**
 * Create a `LessonStore` backed by the provided `EngineerStoreReader`.
 *
 ...
 */
export function createJsonlLessonStore(reader: EngineerStoreReader): LessonStore {
  return {
    // No-op: recording is owned by appendSignal in engineer-store.ts (FR-5).
    ...
    async retrieve(query: LessonQuery): Promise<RetrievedLesson[]> {
      ...
      const byRecencyDesc = (a: EngineerSignal, b: EngineerSignal): number =>
