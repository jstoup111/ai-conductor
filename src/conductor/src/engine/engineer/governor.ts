 ...
 *
 * `governorReport` reads the engineer signal store via a `EngineerStoreReader`,
 ...
 *     upstream `createEngineerStoreReader` returns `[]` for missing/unreadable
...
/**
 * Compute a read-only aggregate governor report from the engineer signal store.
 *
 ...
 *
 * @param reader  A `EngineerStoreReader` (e.g. from `createEngineerStoreReader`).
