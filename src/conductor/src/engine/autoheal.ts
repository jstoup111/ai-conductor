 ...
 * authoritative source of task completion — the trailer/sidecar-evidence
 * derivation engine that superseded it (feature #773, Task 11) has itself
 * since been deleted; commit-trailer stamping is telemetry only now, and
 * task-status.json rows are the sole source of truth for completion.
 * attemptAutoHeal is retained solely as a best-effort migration aid for
 * pre-cutover repos that have no trailer
