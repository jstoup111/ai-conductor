...
import { join as join19, basename as basename3, dirname as dirname4, relative } from "path";
...
var FileSpanExporter = class {
  ...
  ensureDir() {
    if (!this.dirEnsured) {
      mkdirSync2(dirname5(this.filePath), { recursive: true });
      this.dirEnsured = true;
    }
  }
};
...
async function buildDaemonModeOptions(projectRoot, daemonCmd, deps = { sessionNameForRepo, hasSession, respawnPane }) {
  const sessionName = deps.sessionNameForRepo(projectRoot);
  const triggerSelfRestart = await deps.hasSession(sessionName) ? async () => {
...
async function main() {
  ...
  if (daemonSupervisorCmd) {
    const { dispatchDaemonSupervisor } = await import("./daemon-supervisor-cli-HZTZCWGQ.js");
