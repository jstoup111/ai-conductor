...
import { open, mkdir, unlink, readFile, writeFile } from "fs/promises";
...
async function ensureRunning(repoPath, opts = {}) {
  ...
  const launchFn = opts.launch ?? (async (path) => {
    const { launchDaemon } = await import("./daemon-launch-V24BBKVK.js");
    launchDaemon(path);
  });
...
import { mkdir as mkdir3, readFile as readFile3, rm, writeFile as writeFile3 } from "fs/promises";
import { join as join4 } from "path";
var RESTART_MARKER = ".daemon/RESTART-PENDING";
async function writeRestartPending(projectRoot, opts = {}) {
  await mkdir3(join4(projectRoot, ".daemon"), { recursive: true });
  const intent = {
    requestedAt: (/* @__PURE__ */ new Date()).toISOString(),
    ...opts.requestedBy !== void 0 ? { requestedBy: opts.requestedBy } : {},
    ...opts.blockingSlug !== void 0 ? { blockingSlug: opts.blockingSlug } : {}
  };
  await writeFile3(join4(projectRoot, RESTART_MARKER), JSON.stringify(intent, null, 2), "utf-8");
}
async function readRestartPending(projectRoot) {
  let raw;
  try {
    raw = await readFile3(join4(projectRoot, RESTART_MARKER), "utf-8");
  } catch {
    return null;
  }
  try {
...
async function consumeOnBoot(projectRoot) {
  const markerPath = join4(projectRoot, RESTART_MARKER);
  let raw;
  try {
    raw = await readFile3(markerPath, "utf-8");
  } catch {
    return null;
  }
...
//# sourceMappingURL=chunk-4JHATRZD.js.map