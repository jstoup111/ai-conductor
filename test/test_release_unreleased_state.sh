...
assert_unreleased_migration_command() {
  local description=$1
  local changelog=$2
  local migration
  migration=$(awk '
  ...
  if printf '%s\n' "$migration" \
    ...
    printf 'FAIL %s (Unreleased lacks runnable ./bin/install --update migration)\n' \
      "$description"
    FAIL=$((FAIL + 1))
  fi
}
...
assert_unreleased_migration_command \
  "Unreleased carries the supported install reconciliation migration" \
