...
### Added
- Spec landed for daemon build-start `base_refresh` (`.docs/{track,complexity,stories,plans}/daemon-build-start-base-refresh.md`): a new engine-native, daemon-gated pipeline step that runs `git fetch origin` + rebase of the feature worktree onto `origin/<default>` at the BUILD phase boundary (after `plan`, before `acceptance_specs`/`build`), so every build task is authored against the newest merged code and evidence anchors are created on the already-rebased base — reducing the #535/PR-593 `anchor is unreachable` window instead of re-introducing it. Reuses `resolveBase`/`performRebase`/`runGatedRebaseResolution` verbatim, mirrors the SHIP-time `rebase` step's non-daemon no-op guard, and fails closed (conflict → `/rebase` HALT). Daemon/self-host only; interactive `/conduct` and consumers are unaffected. Sibling of #598 (stale engine binary) — kept separate. Implementation tracked separately; this entry documents the queued spec.
