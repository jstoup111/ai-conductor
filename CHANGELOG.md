...
- Tolerated own-feature DECIDE-artifact amendments are now logged and judged by `build_review`'s Scope rubric, so unjustified changes fail review instead of passing silently ({{IMPLEMENTATION_PR}}).
