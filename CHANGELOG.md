...
- Add `conduct-ts validate-wired-into <plan-file-path> [--cwd <dir>]`, a DECIDE-time gate that resolves every task's `**Wired-into:**` anchor in a plan through the same machinery BUILD-time per-task completion verification uses — `extractWiredIntoContracts` for parsing and `same as Task N` inheritance, `verifyDeclaredSites` for the per-anchor check — so the validator's verdict cannot drift from what the build will accept. It prints a per-task `PASS`/`SKIP`/`FAIL` report and, unlike the advisory `overlap-scan`, exits 1 on any failure: a malformed declaration, a `**Wired-into:**` line declaring no call site, an unresolvable inheritance target, a declared file that does not exist, or a symbol with no non-test reference in its declared file. Until now the anchor grammar was enforced only by prose in `skills/plan/SKILL.md` §5c, and an anchor that could never resolve produced no authoring-time error at all — a 19-task plan spun for hours at 0/19 tasks completed while ~25 substantive commits landed, because per-task verification silently never matched. §5c and the `plan` verification checklist now require the command to run against the authored plan, and treat any `FAIL` as a hard block ({{IMPLEMENTATION_PR}}).
