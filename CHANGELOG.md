...
- Every HALT the daemon writes now carries a `.pipeline/HALT.class` disposition — the re-kick sweep used to treat a missing or unrecognized class the same as a safely-retryable one, so a HALT written by a call site that skipped classification (or written before the sidecar existed) could be silently re-kicked instead of retained for an operator; classification is now written atomically and total across every writer, a one-time startup migration stamps pre-existing HALTs `legacy`, and both `unclassified` and `needs-human` dispositions are retained on every sweep ({{IMPLEMENTATION_PR}}).
