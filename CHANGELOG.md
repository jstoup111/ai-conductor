...
- Let durable finish verification tolerate one bounded GitHub PR-head propagation delay after the
  post-finish shipped-record push, while persistent or unrelated shipment mismatches still fail closed
...
- Keep setup-failure triage from disrupting other worktrees by starting missing Docker containers without stopping, restarting, or tearing down running shared services ([implementation PR #1218](https://github.com/jstoup111/ai-conductor/pull/1218)).
...
- DECIDE-flow spec pull requests now carry the `spec` label from the moment they are created ([implementation PR #1197](https://github.com/jstoup111/ai-conductor/pull/1197)).
- Aggregate BUILD verification now runs as deterministic engine machinery instead of a shipped
...
- Every HALT the daemon writes now carries a `.pipeline/HALT.class` disposition — the re-kick sweep used to treat a missing or unrecognized class the same as a safely-retryable one, so a HALT written by a call site that skipped classification (or written before the sidecar existed) could be silently re-kicked instead of retained for an operator; classification is now written atomically and total across every writer, a one-time startup migration stamps pre-existing HALTs `legacy`, and both `unclassified` and `needs-human` dispositions are retained on every sweep ([implementation PR #1148](https://github.com/jstoup111/ai-conductor/pull/1148)).
...
- Stop an in-flight daemon feature after its active serial step or parallel group settles, preserving natural statuses and resuming from the durable boundary after unpark ([implementation PR #1186](https://github.com/jstoup111/ai-conductor/pull/1186)).
...
- The halt-PR reconciliation sweep now clears a stale marking instead of treating it as conforming.
  ...
  ([implementation PR #1146](https://github.com/jstoup111/ai-conductor/pull/1146)).
- The engine no longer dispatches a step into a working directory that no longer exists. The
  ...
  against the same absent path. Nothing is written back into the missing path ([implementation PR #1146](https://github.com/jstoup111/ai-conductor/pull/1146)).
...
- Preserve originating GitHub issue assignees throughout the engineer lifecycle so completion write-back cannot change feature ownership ([implementation PR #1140](https://github.com/jstoup111/ai-conductor/pull/1140)).
...
- Halt daemon-mode kickbacks that target DECIDE as `needs-human` instead of reopening human-judgment steps, while interactive amendment kickbacks continue routing unchanged ([implementation PR #1119](https://github.com/jstoup111/ai-conductor/pull/1119)).
...
- Codex now declares no session-resume capability, so every Codex dispatch is a cold start rather than a
  `codex exec resume` invocation — including within-step retries, which carry their context via the
...
- Build progress display is now 1-based: the first in-progress task shows `1/N`
  ...
  this only affects the rendered display string in `daemon-cli.ts` and
  `create-renderer.ts` via a new pure `displayBuildPosition` helper.
