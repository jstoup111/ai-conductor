...
- Daemon triage now treats `.pipeline/phase-active` and daemon status as advisory context that can warn but never block read-only recovery diagnosis, while every recovery mutation still requires individual operator approval ({{IMPLEMENTATION_PR}}).
