...
### Added
- Spec landed for config-driven daemon build-start base refresh (`.docs/{track,complexity,stories,plans}/daemon-build-start-base-refresh.md`): a new opt-in, per-project `build_start_base_refresh` key in `.ai-conductor/config.yml` (top-level boolean, read at daemon startup like `auto_restart_on_stale_engine`; absent/false → off) that makes the daemon run `git fetch origin` + rebase of the feature worktree onto `origin/<default>` at the BUILD boundary — before the first BUILD-phase step dispatches and before any evidence commit exists — so every build task is authored against the newest merged code and evidence anchors are created on the already-rebased base, reducing the #535/PR-593 `anchor is unreachable` window instead of re-introducing it. Reuses `resolveBase`/`performRebase`/`runGatedRebaseResolution` verbatim via a daemon-only, run-once guard (NO new `StepName`, no config-map churn), and fails closed (conflict → `/rebase` HALT; no-origin → clean no-op). Daemon-only, opt-in; interactive `/conduct` and consumers are unaffected. Sibling of #598 (stale engine binary) — kept separate. Implementation tracked separately; this entry documents the queued spec.
