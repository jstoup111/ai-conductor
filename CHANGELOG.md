...
- Daemon triage now treats a leftover `.pipeline/phase-active` marker as advisory and refuses only when `conduct-ts daemon status` corroborates the recorded live step, allowing read-only recovery diagnosis after daemon crashes ({{IMPLEMENTATION_PR}}).
